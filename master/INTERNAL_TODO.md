...
## OPEN TASK INDEX (priority order, post-2026-04-29 AM audit — 21st pass)
...
- **#134** [GROWTH] [XS] (Epic E2 Trial → Paid Conversion) — Hours-remaining live JS countdown on day-1 trial-urgent banner. Today the day-1 banner renders "Last day of your Pro trial — add a card before midnight..." — but "midnight" is server-time-relative and the user has no concrete countdown clock. Adding a small Alpine `x-data="{ remaining: '' }"` ticker that shows "Trial ends in 14h 23m" recomputed every 60s sharpens the urgency from abstract to concrete. Compounds with #133 (the price anchor) and the existing red urgency styling — together the banner now anchors price ($99/year), time (14h 23m), and consequence (payment links + reminders pause). Implementation: pass `trial_ends_at` (already on `users` row) into the dashboard render scope; client-side computes the diff each minute. Distinct from #102 (per-user timezone — a dependency, not a duplicate; the countdown should respect the user's local timezone if #102 lands first, but functionally falls back to UTC midnight which matches the cron's 09:00 UTC schedule, so no hard block). MED conversion at the trial-end moment. ~20 lines + 2 test assertions.
