...
> **Audited:** 2026-04-28 PM-2 (Task Optimizer cycle, 11th pass). This cycle's deltas: (a) **#45 closed** — last-day urgency dashboard banner shipped (`views/dashboard.ejs` day-1 branch: red panel + role=alert + "Last day" copy + ⏱ emoji + `data-trial-urgent` attribute). 2 new + 1 retired tests in `tests/trial.test.js`. (b) **5 new [GROWTH] items (#86-#90)** from this cycle's Growth Strategist pass: #86 `/vs/<competitor>` comparison pages [S, MED-HIGH SEO acquisition — distinct from #25 niche, TODO_MASTER #26 G2]; #87 Stripe payout reconciliation widget [S, MED-HIGH retention via accounting moat — distinct from #64 aging receivables, #76 QBO/Xero export]; #88 "Frequent non-payer" client-pattern alert [XS, MED retention via unique signal]; #89 Vacation mode toggle [S, MED retention via seasonal-cancellation closure]; #90 Re-engagement email for 60+ day dormant free users [XS, MED-HIGH free→paid via reactivation — distinct from #11 paid-cancelled, #29 trial, #80 Pro digest]. (c) **2 new [MARKETING] items in TODO_MASTER (#52-#53)** — comparison-page outreach to "best invoicing 2026" listicle authors (companion to #86) + moderator-targeted Slack/Discord outreach for vacation-mode launch (companion to #89). (d) **Test Examiner pass:** added `testNewInvoiceRouteSurvivesRecentClientsNonArrayResult` to `tests/recent-clients.test.js` — 18 assertions across 6 non-array helper-result variants exercising the `Array.isArray ? rows : []` defensive branch. (e) **UX direct fixes** — `views/settings.ejs` upgrade CTA "Upgrade to Pro Monthly/Annual →" replaced with "Start 7-day free trial →" + "No credit card required. Cancel anytime." subcopy (alignment with `views/pricing.ejs` + `views/partials/upgrade-modal.ejs`). New `views/not-found.ejs` + `server.js` 404 handler — silent `redirect('/')` dead-end replaced with a real 404 page returning HTTP 404 + `noindex,nofollow` + adaptive CTA (authed → `/invoices`, anon → `/`). (f) Cross-overlap check on the 5 new GROWTH items vs all 85 prior items + 51 prior TODO_MASTER items: ✓ no duplicates introduced. Full suite now **329 passing, 0 failing** (was 322; +7 net new tests this cycle).
>
> Cross-checks for non-overlap on this cycle's adds: #86 vs #25 (different intent surface — comparison vs vertical fit) and vs TODO_MASTER #26 (different funnel — first-party editorial vs aggregator listing); #87 vs #64 (different time horizon — pre-payment outstanding vs post-payment cash-flow) and vs #76 (different cadence — in-month live vs batch-period export); #88 vs #64 (different signal — historical pattern vs current outstanding) and vs #16 (different audience — analytical alert vs client-side dunning); #89 vs #21 (different surface — freelancer-side toggle vs client-side site) and vs #11 (different timing — pre-cancellation vs post-cancellation); #90 vs #11 (different cohort — never-paid free vs cancelled-paid), vs #29 (different cohort — long-dormant free vs active trial), vs #80 (different cohort — free vs Pro). TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image), #39 (APP_URL) all remain genuinely open. No items flip to [LIKELY DONE - verify] this cycle.
>
> **10th-pass audit retained:** Audited 2026-04-28 (Task Optimizer cycle, 10th pass). This cycle's deltas: (a) **H14 closed** — `escapeHtml`/`formatMoney` consolidated to new `lib/html.js`; three call sites (`lib/email.js`, `jobs/reminders.js`, `jobs/trial-nudge.js`) now require from the shared module. `tests/html-helpers.test.js` adds 27 assertions including 3 cross-module reference-equality checks that hard-fail any future commit re-introducing a private copy. Defence-in-depth bonus: prototype-pollution guard via `Object.prototype.hasOwnProperty` for currency-symbol lookup (pre-fix, `formatMoney(1, '__proto__')` returned `[object Object]1.00`; post-fix returns `1.00` unknown-currency fallback). (b) **5 new [GROWTH] items (#81-#85)** from this cycle's Growth Strategist pass: #81 "Add a payment link before sending?" Pro guard for $100+ drafts [S, MED-HIGH conversion lift on time-to-payment — distinct from #15 by being a single high-value guardrail]; #82 plan comparison table on dashboard for free users [XS, MED conversion — reinforces #31 progress bar]; #83 stale-draft dashboard nudge [S, MED activation/revenue recovery — distinct from #16/#29]; #84 auto-generate plain-language email body using first-line-item summary on send [XS, MED activation — applies to ALL plans, distinct from #68 Pro template editor]; #85 URL-params dynamic landing-hero copy `?for=&trade=` [S, MED-HIGH SEO long-tail — distinct from #25/#36/#56/#52]. (c) **2 new [MARKETING] items in TODO_MASTER (#50-#51)** — Indie Hackers product directory + 4-week "Building in Public" series [distinct from #12 PH, #19 Show HN, #20 newsletters], single-spot freelance-podcast test buy [distinct from #15/#18/#20]. (d) **UX direct fix** — `views/invoice-view.ejs` action bar: when `invoice.status='draft'`, "📤 Mark as Sent" is now the brand-coloured primary CTA (was gray-bordered secondary); "Print / Download PDF" demoted to secondary on draft + renamed to "📄 View / Download PDF". Sent/overdue/paid states unchanged. Inverts the previous incorrect visual hierarchy where the lifecycle CTA was quieter than the static Print button. (e) Cross-overlap check on the 5 new GROWTH items vs all 80 prior items + 49 prior TODO_MASTER items: ✓ no duplicates introduced.
>
> Cross-checks for non-overlap on this cycle's adds: #81 vs #15 (#15 = cross-app contextual upsell prompts on locked features, #81 = single specific guardrail at the highest-intent moment in the invoice lifecycle — different scope); #82 vs #31 progress bar (#31 = visual cap progress, #82 = feature-level table) and vs `views/pricing.ejs` (different surface: dashboard vs marketing page); #83 vs #16 (#16 = client-facing reminders on already-sent invoices, #83 = freelancer-facing nudge on never-sent drafts) and vs #29 (different lifecycle: #29 = trial-end-day-3 user-facing email, #83 = dashboard banner for stale drafts); #84 vs #68 (different model: #68 = Pro-only manual template editor, #84 = auto-generated default copy for ALL plans); #85 vs #25 (different mechanism: #25 = static .ejs files per niche, #85 = dynamic param-driven hero on the bare `/` route) and vs #36/#56/#52 (different concerns: #85 is in-page hero copy + dynamic title/canonical vs OG metadata / robots / JSON-LD). Each add was also re-checked against the [LIKELY DONE] gate in TODO_MASTER (none flip this cycle) and against the prior cycle's #76-#80 to confirm no thematic drift.
>
> Prior context preserved (compacted to keep header readable):
> 9th pass shipped #31 closure + 5 [GROWTH] (#76-#80) + 1 [MARKETING] (TODO_MASTER #49) + 6 regression tests in `tests/recent-regression.test.js`. **Audit metadata still applies:** archive trigger remains 1.5k lines, currently at ~2.4k after this cycle (overdue by 6 cycles — bumped to next cycle once again; not blocking work, just hygiene); priority order unchanged: **[TEST-FAILURE] (none) > income-critical features > [UX] items that affect conversion > [HEALTH] > [GROWTH] > [BLOCKED]**; complexity tags: [XS] < 30 min · [S] < 2 hrs · [M] 2–8 hrs · [L] > 8 hrs. TODO_MASTER reviewed: #38 (OG image), #39 (APP_URL), #18 (Resend API key) all remain genuinely open. Full suite now **37 files, 251 test functions, 0 failures** (+27 this cycle).
>
