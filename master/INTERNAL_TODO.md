...
> **Audited:** 2026-04-28 PM (Task Optimizer cycle, 16th pass). This cycle's deltas: (a) **#106 closed** — trial-countdown timer pill shipped in `views/partials/nav.ejs`. New `formatTrialCountdown(trialEndsAt, now?)` helper in `lib/html.js` (returns `{days, hours, label, urgent}` or null); wired through `res.locals.trialCountdown` in `server.js` middleware so every authed page renders the pill, not just the dashboard. `routes/auth.js` login + register now persist `trial_ends_at` + `subscription_status` on session.user; `routes/invoices.js` dashboard refresh keeps it fresh on each visit. Two-branch styling (red/⏱ when <24h, amber/⏳ otherwise). Pill is `hidden sm:inline-flex` to give mobile breathing room. 25 new tests in `tests/trial-countdown-nav.test.js` (12 helper unit + 10 nav render + 3 wiring), all passing. Full suite is 44 files, 0 failures. (b) **5 new [GROWTH] items (#112-#116)** from this cycle's Growth Strategist pass: #112 live "Trusted by N freelancers in M countries" hero counter [XS, MED conversion via dynamic social proof]; #113 per-niche `<meta description>` blurb [XS, MED SEO snippet-CTR lift]; #114 minimum-viable referral attribution `?ref=<slug>` [S, MED-HIGH virality; pre-cursor to #18]; #115 "Reply to client" mailto chip on paid invoices [S, MED retention via rebook lift]; #116 dashboard empty-state demo accordion [XS, MED activation]. (c) **1 new [MARKETING] item in TODO_MASTER (#58)** — submit to 6 SaaS-comparison directories (Slant / AlternativeTo / SaaSHub / Capiche / StackShare / Tekpon) — passive evergreen discovery channel distinct from Product Hunt (event), G2/Capterra (review-driven), AppSumo (transactional). (d) **UX direct fixes (2 items)** — `views/auth/login.ejs` "No account? Sign up free" → "New to QuickInvoice? Create a free account →" (clearer CTA, less yes/no question framing); `views/dashboard.ejs` fallback path "0/3 invoices used" → "0 of 3 invoices used" (less fraction-y, plain English).
>
> Cross-checks for non-overlap on this cycle's adds: #112 vs #20 social proof testimonial wall (testimonials are qualitative + static; this is quantitative + dynamic + impossible-to-fake-feeling); #113 vs #36 OG/Twitter Card metadata (different cohort: organic-search SERP vs paid-social shares); #114 vs #18 full referral program (different scope: attribution-only vs Stripe-coupon-driven; #114 is the data-layer pre-cursor that unlocks #18); #115 vs #55 auto thank-you (different sender + lifecycle moment: Stripe receipt vs human freelancer rebook lever); #116 vs #60 demo-mode (different surface: empty-state passive video vs `/demo` interactive sandbox at standalone URL). MARKETING #58 vs #26 G2/Capterra (review-driven, gravity-dependent vs profile-completeness-driven), vs #12 PH (24-hour event vs evergreen passive), vs #57 AppSumo (transactional cash-event vs passive evergreen referral surface). TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL) all remain genuinely open. No items flip to [LIKELY DONE - verify] this cycle.
>
...
## OPEN TASK INDEX (priority order, post-2026-04-28 PM audit — 16th pass)
...
**Income-critical [GROWTH] — XS first (highest impact-per-effort)**
  *(**#106 closed 2026-04-28 PM** — trial-countdown pill shipped in `views/partials/nav.ejs` rendering on every authed page (not just the dashboard banner). New `formatTrialCountdown(trialEndsAt, now?)` helper in `lib/html.js` returns `{days, hours, label, urgent}` or `null` (null = no pill). Wired through `res.locals.trialCountdown` middleware in `server.js`; `routes/auth.js` login + register persist `trial_ends_at` + `subscription_status` on `req.session.user`; `routes/invoices.js` dashboard refresh keeps both fresh. Two-branch styling: amber `bg-amber-50/text-amber-800` + ⏳ when ≥24h remain (calm), red `bg-red-50/text-red-700` + ⏱ when <24h (urgent — same emoji as the dashboard last-day banner for consistency). Label format: `Xd Yh left` / `Yh left` / `<1h left`. Pill is `hidden sm:inline-flex` (yields to Invoices/Settings/Logout on the smallest mobile) and links to `/billing/upgrade?cycle=annual&utm=nav-countdown` so paid conversions from the persistent pill are attributable in analytics distinct from #45 banner clicks. 25 new tests in `tests/trial-countdown-nav.test.js` (12 helper unit + 10 nav.ejs render + 3 wiring/integration), all passing. Full suite: 44 files, 0 failures. MED conversion lift via persistent surface — every page-view during the trial now carries the countdown rather than only the dashboard.)*
- **#107** [S] — Last-30-day paid-revenue stat row above the dashboard invoice list. Pulls `SUM(amount) WHERE status='paid' AND paid_at >= NOW() - INTERVAL '30 days'` for the user, plus a separate count of distinct paying clients. Renders as 3 stat cards: "$X paid in last 30 days", "Y clients paid you", "Z invoices paid". Distinct from #87 payout reconciliation (different scope: payout = what Stripe is sending you next; this is a forward-looking growth metric for the freelancer's own use). Distinct from #64 aging receivables (which is what's outstanding, not what's been collected). MED retention — the Stripe-Atlas effect: showing the user their own MRR-style metric makes the product feel like a partner in their growth, not just an invoicing tool. Compounds with #88 frequent-non-payer alert (negative signal) by adding a positive signal to the same dashboard row. New SELECT in `db.js` (`getRecentRevenueStats(userId, days=30)`), new section in `views/dashboard.ejs` above the table.
