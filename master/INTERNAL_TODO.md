...
> **Audited:** 2026-04-28 AM (Task Optimizer cycle, 15th pass). This cycle's deltas: (a) **#101 closed** — inline annual-savings $-amount pill shipped across all three Monthly/Annual toggle surfaces (`views/pricing.ejs`, `views/settings.ejs`, `views/partials/upgrade-modal.ejs`). Pure view change. 11 new tests in `tests/annual-savings-pill.test.js`, all passing. (b) **6 new [GROWTH] items (#106-#111)** from this cycle's Growth Strategist pass: #106 trial-countdown timer in global nav [XS, MED conversion]; #107 last-30d paid-revenue stat row above dashboard [S, MED retention dopamine]; #108 on-pricing-page side-by-side competitor mini-table [S, MED conversion]; #109 sticky "+" FAB on mobile dashboard [XS, MED activation]; #110 passwordless magic-link login [M, MED conversion + retention; Resend-blocked]; #111 "Show client preview" pre-send modal on invoice-form [S, MED activation; gated on #43]. (c) **1 new [MARKETING] item in TODO_MASTER (#57)** — apply to AppSumo for a lifetime-deal listing (5-figure cash-injection lever in a 60-day window — distinct cash-event pattern from every other [MARKETING] item; gated on #58 redemption page + LTD tier in `db/schema.sql` + 5 testimonials on landing). (d) **Test Examiner pass:** new `tests/annual-savings-pill.test.js` (11 tests) — closed conversion-pill coverage gap; full suite is 43 files, 0 failures. (e) **UX direct fixes (3 items)** — `views/index.ejs` Pro card now shows `save $45/year` (was `save 31%`); `views/invoice-form.ejs` Due Date hint rewritten to plain English (`(defaults to 30 days)` was `(Net 30 default)`); `views/dashboard.ejs` Pro plan-line now also names "& payment links" feature (was `Pro plan · Unlimited invoices`). (f) Cross-overlap check on the 6 new GROWTH items vs all prior items: ✓ no duplicates.
>
> Cross-checks for non-overlap on this cycle's adds: #106 vs #45 (closed dashboard banner — different surface); #107 vs #87 (different scope — payout vs MRR-style metric); #108 vs #86 (different surface — at-decision-moment vs SEO landing page); #109 vs #91 (different action) / vs #23 (different surface — install-time vs in-app); #110 vs #17 Google OAuth (different fork — magic link works for any email); #111 vs PDF preview (different artefact — client-facing HTML vs print PDF) / vs #43 (different role — URL itself vs in-form preview); MARKETING #57 vs #12 PH (different windows — 24hr launch vs 60-day deal) / vs G2/Capterra (different funnel position — review/discovery vs transactional). TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL) all remain genuinely open. No items flip to [LIKELY DONE - verify] this cycle.
>
...
## OPEN TASK INDEX (priority order, post-2026-04-27 PM-5 audit — 14th pass)
...
- **#106** [XS] — Trial-countdown timer in `views/partials/nav.ejs` for users in `subscription_status='trialing'`. Today the trial urgency only surfaces on the dashboard banner (#45 — closed) and on the trial-nudge email (#16). Every other authed page (invoice-view, invoice-form, settings, pricing, /redeem) is "trial-countdown silent" — a Pro user mid-trial editing their 3rd invoice on day 6 sees no countdown anywhere. Add a small persistent pill (`Xd Yh left in trial · Add card`) right of the nav links, linking to `/billing/upgrade?cycle=annual&utm=nav-countdown`. Pure server-side render (compute `trial_ends_at - NOW()` on each page-load — no client tick needed; precision to the hour is sufficient and avoids Alpine timer complexity). Persistent surface increases conversion-per-trial-day proportionally to page-views per session. MED conversion. Distinct from #45 dashboard banner (different surface, different page coverage), distinct from email nudges (different channel, different cognitive moment). Pure view + 1 helper function in `lib/html.js` (`formatTrialCountdown(trial_ends_at)`).
