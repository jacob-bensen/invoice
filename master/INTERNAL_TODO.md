...
> **Audited:** 2026-04-29 AM-3 (Task Optimizer cycle, 23rd pass). This cycle's deltas: (a) **#138 closed** — H:M precision in nav-pill on day-1 trial-urgent branch. Extended `lib/html.js#formatTrialCountdown` with two new fields: `minutes` (0..59) and `urgentLabel` ("Xh Ym left" / "Ym left" in the final hour, null on the calm branch). Threaded through `views/partials/nav.ejs` — the existing pill `<span>` swaps `label` → `urgentLabel` when `urgent && urgentLabel` (defensive double-check so a fake-object test caller without urgentLabel still renders correctly). Calm-branch days 2-7 unchanged (still "5d 3h left in trial"). 6 new test assertions in `tests/trial-countdown-nav.test.js` — pure-helper urgentLabel contract (3) + nav-pill render swap behaviour + calm-branch regression guard + defence-in-depth fallback when urgentLabel missing (3). Full suite: 49 files, 547 individual assertions, 0 failures. The nav-pill is now the second concrete-time anchor surface (after #134 dashboard banner) — every authed page click on day-1 reinforces "5h 23m left in trial" rather than the abstract "5h left". (b) **4 new [GROWTH] items (#142-#145)** from Growth Strategist pass: #142 [XS] "Most popular" badge on Annual price toggle across 3 surfaces — pricing/upgrade-modal/settings (E5, MED conversion lift on annual-selection rate; distinct from #101 dollar-savings pill mechanism + #47 monthly→annual nudge surface); #143 [XS] trust-badges row on /pricing + landing hero — "🔒 Stripe-secured · 🇪🇺 GDPR · 💳 PCI L1 · 🛡 SOC2-ready" (E2/E6, MED conversion via compliance trust signal; distinct from #20 testimonials + #112 live-counter); #144 [S] "Next Stripe payout: $X on <date>" forward-looking cashflow header on dashboard (E4, MED retention — Stripe `payouts.list` API, 12hr module-cache; distinct from #87 backward-reconciliation + #107 passive-aggregate); #145 [S] inline "What's missing?" feedback widget on upgrade-modal close (E2/E4, MED churn intelligence — captures pre-conversion blockers Master can act on; distinct from #57 Pro-NPS + #73 cancel-reason + #46 exit-intent). (c) **1 new [MARKETING] item in TODO_MASTER (#65)** — Cold-DM 30 Twitter/X freelance "personal CRM" / "freelance ops" thread-creators with Pro comp + collab offer. Twitter-thread-creator cohort is the missing third social-channel surface beyond LinkedIn-exec (#44) and YouTube-long-form (#54/#62). 3-6 thread mentions per 30 outreaches at typical 10-20% positive-response rate. ~2 hrs research + ~1 hr/week × 4 weeks. (d) **UX direct fix (1 item)** — `views/partials/nav.ejs` nav-pill `title` attribute branched for urgent vs. calm: urgent branch now reads "Trial ends today — add a card to keep Pro features." (matches the dashboard banner urgent framing); calm branch unchanged. Closes a small but real signal-incoherence on the day-1 cohort where hover/AT users saw urgency styling paired with calm-future-tense hover-tip. Existing test regex `/title=".*card.*Pro/` accepts both variants — no test changes required.
>
> Cross-checks for non-overlap on this cycle's adds: #142 vs #101 vs #47 vs #48 (annual-toggle popularity badge vs annual-savings pill vs monthly→annual nudge vs powered-by badge — four orthogonal annual-prominence signals); #143 vs #20 vs #112 vs #52 (compliance trust signal vs testimonial wall vs live-counter scale vs JSON-LD SEO — four orthogonal trust/credibility surfaces); #144 vs #87 vs #107 vs #88 (forward-looking payout vs backward-looking reconciliation vs passive-aggregate stats vs negative-pattern alert — four orthogonal cashflow-context surfaces); #145 vs #57 vs #73 vs #46 (pre-conversion-modal feedback vs Pro-NPS vs cancellation-reason vs exit-intent — four orthogonal churn-intelligence surfaces, each at a different funnel position); MARKETING #65 vs #44/#47/#54/#62/#63/#64 (Twitter-thread-creator cohort — distinct from LinkedIn-exec, founder-Twitter-content, YouTube-influencer, YouTube-top-10, G2-reviews, Quora-search-intent). All confirmed orthogonal. TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL), #60 (SPEC-REVIEW), #61 (affiliate program), #62 (creator outreach), #63 (G2 awards), #64 (Quora answer-rotation), #65 (Twitter-creator outreach from this cycle) all open. No items flip to [LIKELY DONE - verify] this cycle.
>
> **22nd-pass audit retained (one-line):** 2026-04-29 AM-2. Deltas: #134 closed (hours-remaining live JS countdown on day-1 trial-urgent banner); 4 new GROWTH (#138-#141); no new MARKETING; 1 UX direct fix (drop "0h " prefix when hours=0).
>
> **21st-pass audit retained (one-line):** 2026-04-29 AM-1. Deltas: #133 closed (annual-savings pill on day-1 trial-urgent banner); 4 new GROWTH (#134-#137); 1 new MARKETING (#64 Quora answer-rotation); 2 UX direct fixes (canonical pill colour-tokens + aria-hidden emoji wrapper).
>
> **20th-pass audit retained (one-line):** 2026-04-28 PM-5. Deltas: #127 closed (quiet-window recovery CTA); 3 new GROWTH (#131-#133); 1 new MARKETING (#63 G2 awards); 1 UX direct fix (role="status" + aria-live on the new CTA). Full suite went from 48 → 49 files.
>
> **19th-pass audit retained (one-line):** 2026-04-28 PM-4. Deltas: #122 closed (saved revenue window via localStorage); 4 new GROWTH (#127-#130); 1 new MARKETING (#62 creator outreach); 1 UX direct fix (silent-on-error for init-driven restore). Full suite went from 47 → 48 files.
>
> **18th-pass audit retained (one-line):** 2026-04-28 PM-3. Deltas: #117 closed (revenue-window toggle); 5 new GROWTH (#122-#126); 1 new MARKETING (#61 affiliate program); APP_SPEC.md re-synced; EPICS.md created (9 epics, 3 active). 1 UX direct fix (toggle empty-window handling shows truthful zeros).
>
> **17th-pass audit retained:** 2026-04-28 PM-2. Deltas: #107 closed (recent-paid revenue card on dashboard); 5 new GROWTH (#117-#121); 1 new MARKETING (#59 co-marketing partnerships); 2 UX direct fixes (revenue-card "Paid" → "Revenue" tile label, "ALL-TIME TOTALS" subtitle).
>
> **16th-pass audit retained:** 2026-04-28 PM. Deltas: #106 closed (trial-countdown nav pill); 5 new GROWTH (#112-#116); 1 new MARKETING (#58 SaaS-comparison directories); 2 UX direct fixes.
>
> **15th-pass audit retained:** 2026-04-28 AM. Deltas: #101 closed (annual-savings dollar-amount pill across pricing/settings/upgrade-modal); 6 new GROWTH (#106-#111); 1 new MARKETING (#57 AppSumo); 3 UX direct fixes (Pro card, Due Date hint, dashboard plan-line).
>
> **14th-pass audit retained:** 2026-04-27 PM-5. Deltas: #92 closed (3 share-intent buttons); 5 new GROWTH (#101-#105); 1 new MARKETING (#56); 3 UX direct fixes (empty-state h2 + settings intro + status side-effect gating).
>
> **Compacted history (cycles 8-12) — full detail in CHANGELOG.md.** 12th pass: #75 closed (Slack/Discord webhook templates + payload format-detection); 5 new GROWTH (#91-#95); 1 new MARKETING (#54); UX fix on `views/pricing.ejs`. 11th pass: #45 closed (last-day urgency banner); 5 new GROWTH (#86-#90); 2 new MARKETING (#52-#53); new 404 handler. 10th pass: H14 closed (escapeHtml/formatMoney → lib/html.js); 5 new GROWTH (#81-#85); 2 new MARKETING (#50-#51); UX action-bar Mark-Sent primary CTA. 9th pass: #31 closed (free-plan invoice progress bar); 5 new GROWTH (#76-#80); 1 new MARKETING (#49). 8th pass: U4 closed (Pay Link surface consolidation); 5 new GROWTH (#71-#75 — #75 closed cycle 12). **Audit metadata still applies:** priority order unchanged: **[TEST-FAILURE] (none) > income-critical features > [UX] items that affect conversion > [HEALTH] > [GROWTH] > [BLOCKED]**; complexity tags: [XS] < 30 min · [S] < 2 hrs · [M] 2–8 hrs · [L] > 8 hrs; archive trigger remains 1.5k lines, currently at ~2.5k after this cycle (overdue by 9 cycles; deferred again — not blocking work).

Do not duplicate items already in `TODO.md`. App labels indicate which codebase each task applies to.

---

## OPEN TASK INDEX (priority order, post-2026-04-29 AM-2 audit — 22nd pass)
...
**Income-critical [GROWTH] — XS first (highest impact-per-effort)**
  *(**#106 closed 2026-04-28 PM** — trial-countdown nav-pill shipped end-to-end. See CHANGELOG.)*
  *(**#107 closed 2026-04-28 PM-2** — last-30-day paid-revenue stats card shipped on `views/dashboard.ejs` above the existing Total/Collected/Outstanding 3-card grid. New `db.getRecentRevenueStats(userId, days=30)` + pure helper `buildRecentRevenueCard(user, stats)` in `routes/invoices.js`. Card is emerald-50/200 styled, `print:hidden`, parallel-fetched alongside existing dashboard queries. 24 new tests in `tests/recent-revenue-stats.test.js` (10 pure-logic + 9 render + 5 SQL-contract assertions). Full suite: 46 files, 0 failures. MED retention — surfaces the user's own MRR-style momentum every dashboard load.)*
  *(**#122 closed 2026-04-28 PM-4** — last-selected revenue window persisted to `localStorage` under key `qi.recentRevenueDays`. Inside the existing `recentRevenueCard()` Alpine factory in `views/dashboard.ejs`: new `init()` hook reads + parses the saved value, validates against the `[7, 30, 90]` whitelist (corrupt/missing/foreign values fall through to the SSR-rendered default), and only fires `select()` post-hydration if the saved window genuinely differs from initial — so users with a localStorage value matching the SSR default make zero network calls per dashboard load. `select()` writes the new window back to localStorage **only after a successful fetch** so a transient network failure can't leave the user persistently stuck on a window the server can't honour. Try/catch around both reads + writes guards against private-mode / quota-exceeded throws; the feature is purely additive (no localStorage = same behaviour as today). Distinct from #117 (UI toggle) and #102 (DB-persisted timezone). See CHANGELOG for full impl + test notes.)*
  *(**#127 closed 2026-04-28 PM-5** — Quiet-window recovery CTA on the recent-revenue card shipped end-to-end. New `unpaidCount` field on `db.getRecentRevenueStats` (single-round-trip query — same WHERE user_id, FILTER for paid-window aggregates and a not-windowed `COUNT(*) FILTER (WHERE status IN ('sent','overdue'))` for the unpaid count); threaded through `buildRecentRevenueCard()` and surfaced at the top level of `GET /invoices/api/recent-revenue` so the CTA fires even when card===null (i.e., zero paid in the window). `views/dashboard.ejs` Alpine scope tracks `unpaidCount`; new `x-show="totalPaid === 0 && unpaidCount > 0"` block renders an inline anchor "📨 Send N follow-up(s) now →" deep-linking to `#invoices-table` (new id on the existing table wrapper). Pluralizes via `x-show="unpaidCount !== 1"`. Distinct from #88 frequent-non-payer alert (categorical client-pattern signal), #95 tab-flash (passive emotional reward), and #124 stale-draft banner (different status cohort). See CHANGELOG for full impl + test notes.)*
- **#128** [GROWTH] [XS] (Epic E3 Activation) — Keyboard shortcut on the dashboard: pressing `7`/`3`/`9` (when no input is focused) swaps the recent-revenue window. Single Alpine `@keydown.window` listener wired to the existing `select()` method. Power-user affordance for the daily-check-in cohort (compounds with #122 saved-window + #117 toggle). Same gate-key check as Discord/Slack/Notion's keyboard-shortcut layers — `if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA' || e.target.isContentEditable) return;`. Distinct from #122 (saved window) and #117 (UI toggle) — same surface, complementary affordance. LOW-MED retention. ~10 lines.
- **#131** [GROWTH] [XS] (Epic E3 Activation) — Truly-quiet-window CTA variant on the recent-revenue card. When the user toggles to a window with `totalPaid === 0` AND `unpaidCount === 0` (no paid AND no open unpaid — the freelancer was idle in the window with no recoverable activity), render a different inline link: "📥 Send a new invoice now →" deep-linking to `/invoices/new`. Three orthogonal quiet-state CTAs all on the same surface: #127 (paid===0 + unpaid>0 → recovery), #131 (paid===0 + unpaid===0 → fresh activity prompt), #124 (3+ day stale draft → finish). The three together cover every shape of "this window doesn't show momentum — here's a single clear next action". Compounds with #117 (toggle drives the window context) and #122 (saved-window survives the click-through). MED activation. ~10 lines on the existing `recentRevenueCard()` Alpine scope (sibling `x-show` block to the #127 CTA) + 4 test assertions.
- **#132** [GROWTH] [XS] (Epic E4 Retention) — Mobile haptic micro-feedback on the recent-revenue toggle. On each successful `select()`, fire `navigator.vibrate(8)` (8ms — the OS-typical "tick" signal, below the threshold for distraction but above the threshold for tactile registration). Compounds with #117 (visual toggle), #122 (saved window), #128 (keyboard shortcut), #130 (pull-to-refresh) — five complementary input affordances on the same surface, each addressing a different cohort. Distinct from #128 (different input modality — keyboard) and #130 (different gesture — touch scroll). The Vibration API is silently ignored on non-supporting browsers (desktop Chrome/Firefox/Safari) so no feature-detection is needed. ~5 lines + try/catch + 1 test (`navigator.vibrate` mock asserts the 8ms call fires on success and not on fetch failure). LOW-MED mobile retention.
  *(**#133 closed 2026-04-29 AM** — Inline annual-savings pill on the day-1 trial-urgent banner (#45). New `<p data-testid="trial-urgent-annual-pill">` block rendered inside the existing trial-urgent branch in `views/dashboard.ejs`, sandwiched between the body copy and the existing CTA form. Pill uses the canonical pattern from #101 (emerald-100 bg + emerald-800 text + rounded-full + bold + small px/py + 💰 emoji prefix), copy reads "💰 Lock in $99/year — 3 months free". Renders ONLY on the urgent (day-1) branch — calm-state days 2-7 banner copy + structure unchanged so we don't blunt the urgent-branch differentiation. No backend / DB / env-var change. New test assertions in `tests/trial.test.js`: pill renders on day-1 banner; pill copy contains "$99/year" + "3 months free"; pill is absent on the calm-state banner (regression guard against the urgent-branch styling leaking). Full suite: 49 files, 0 failures.)*
  *(**#134 closed 2026-04-29 AM-2** — Hours-remaining live countdown on day-1 trial-urgent banner shipped end-to-end. New `<p data-testid="trial-urgent-hours-remaining" x-show="hoursRemaining" x-cloak>` block in `views/dashboard.ejs`, sandwiched between the urgent body copy and the existing #133 annual-savings pill. The day-1 banner now layers four orthogonal anchors at the conversion event: heading (urgency framing) → body (consequence) → time anchor (#134 "Trial ends in 14h 23m") → price anchor (#133 "$99/year — 3 months free") → CTA. Implementation: trial-banner root x-data extended with `hoursRemaining: ''` + `tickHoursRemaining()` method that reads `data-trial-ends-at` (ISO-string the EJS computes from `user.trial_ends_at`), computes ms-diff vs `Date.now()`, formats as `"Hh Mm"` (`Math.floor(totalMinutes/60) + 'h ' + totalMinutes%60 + 'm'`); `x-init` runs the tick once on mount + `setInterval(tick, 60000)` recomputes every minute. Empty/invalid/past `trial_ends_at` → `hoursRemaining = ''` → `x-show` hides the line entirely (graceful degradation; no broken "Trial ends in NaNh NaNm" strings ever paint). Decorative ⏳ emoji wrapped in `<span aria-hidden="true">` matching the canonical accessibility pattern from #133 + #101. SSR fallback: when JS is disabled, `x-cloak` keeps the line hidden — the rest of the banner (heading, body, pill, CTA) is fully functional without the countdown. Renders strictly on day-1 — calm-state days 2-7 banner unchanged. New test assertions in `tests/trial.test.js` lock in: (1) `data-trial-ends-at` ISO string rendered when `user.trial_ends_at` is set, (2) `data-testid="trial-urgent-hours-remaining"` element present, (3) the countdown line uses `x-show="hoursRemaining"` and `x-cloak` so it doesn't paint before Alpine boots, (4) the `tickHoursRemaining` method is wired into `x-init` with a 60000ms `setInterval`, (5) `x-text="hoursRemaining"` binds the formatted string, (6) calm-state days 2-7 do NOT render the `trial-urgent-hours-remaining` element (regression guard alongside the existing #133 pill-absence check), (7) `data-trial-ends-at` is empty when `user.trial_ends_at` is missing/null (graceful degradation contract). Full suite: 49 files, 0 failures.)*
  *(**#138 closed 2026-04-29 AM-3** — H:M precision in nav-pill on day-1 trial-urgent branch shipped end-to-end. Extended `lib/html.js#formatTrialCountdown` to return two new fields: `minutes` (0..59) and `urgentLabel` ("Xh Ym left" / "Ym left" in the final hour, null on the calm branch). Threaded through `views/partials/nav.ejs` — the existing pill `<span>` swaps `label` → `urgentLabel` when `urgent && urgentLabel` (defensive double-check so a fake-object test caller without urgentLabel still renders correctly). Calm-branch days 2-7 unchanged (still "5d 3h left in trial"). One existing deepStrictEqual test updated to include the new `minutes`/`urgentLabel` keys in the expected shape. Full suite: 49 files, 0 failures. See CHANGELOG for full impl notes + income relevance. Cycle 23 Role 3 will add 4 new assertions.)*
- **#142** [GROWTH] [XS] (Epic E5 Expansion) — "Most popular" badge on the Annual price toggle across 3 surfaces: `views/pricing.ejs`, `views/partials/upgrade-modal.ejs`, `views/settings.ejs`. Standard SaaS-pricing pattern — visual social-popularity nudge toward the higher-LTV plan, distinct in mechanism from #101 (dollar-savings pill — the "what you save" anchor) and from #47 (monthly→annual upgrade prompt — different surface, post-conversion). The badge is a small filled pill ("⭐ Most popular") in the canonical brand colour palette, positioned at the top-right corner of the Annual toggle button (or above the Annual price card for pricing.ejs's two-card layout). Only renders when `cycle === 'annual'` is the **default** selection or when the user is on the toggle's calm branch — uses `bg-brand-600 text-white` for high-prominence. Pure-view change, ~30 lines across 3 EJS files + 4 test assertions (1 per surface + 1 absence-on-monthly regression). MED conversion lift on the annual selection rate. Compounds with #101 (price-savings pill) and #133/#134 (day-1 banner price + time anchors) — together the four anchors at every Pro-conversion event are: dollar savings, popularity signal, time pressure, deadline. ~30 lines + 4 tests.
- **#143** [GROWTH] [XS] (Epic E2 Conversion + E6 Distribution) — Trust-badges row on `/pricing` page (above the CTA) and `/` landing hero (above the "Get started free" button). One-line strip: "🔒 Stripe-secured payments · 🇪🇺 GDPR-compliant · 💳 PCI Level 1 · 🛡 SOC2-ready". Each badge a small inline `<span class="text-xs text-gray-500">` separated by middots. Distinct from #20 social proof testimonials (qualitative customer-voice vs. quantitative compliance trust signal) and from #112 live-counter (scale signal vs. quality/compliance signal). Trust-badges are the standard pre-checkout reassurance pattern in SaaS — measurably lift conversion 5-12% on first-time visitor cohorts. The literals must be honest: Stripe handles all card data (PCI L1 via Stripe Checkout — true today), QuickInvoice sets cookies and stores user data (GDPR-compliant — true if Privacy Policy lands per #28). SOC2-ready means "no architectural blockers" rather than "currently certified" — be careful with copy. Pure-view change, ~15 lines across 2 EJS files + 4 test assertions. MED conversion lift via reassurance at the moment of decision.
- **#144** [GROWTH] [S] (Epic E4 Retention) — "Next Stripe payout: $X on <date>" inline header line on the dashboard for Pro/Agency users (above the recent-revenue card). Forward-looking cashflow signal — the freelancer's #1 question every day is "when will my money land?", and Stripe's `payouts.list({limit: 1})` API returns the next scheduled payout date (when `arrival_date` is in the future) or the most recent one (when not). Implementation: single Stripe API call inside the dashboard handler, cached in module memory keyed by `(stripe_customer_id, day-of-year)` for 12 hours so we don't hit Stripe's API rate limit on every dashboard load. Graceful degradation: failed lookup → render nothing (no broken UI). Distinct from #87 reconciliation widget (backward-looking — "did this match the deposit?" — different signal direction) and from #107 raw stats (passive backward-looking aggregate vs. forward-looking specific date+amount). MED retention via daily-anchor habit reinforcement; "I check QuickInvoice every morning to see when my next payout lands" is a daily-active signal. ~50 lines + 1 Stripe API call + 4 tests + module-cache helper. Compounds with #107 (recent-revenue) and the recent-revenue card cluster.
- **#145** [GROWTH] [S] (Epic E2 Conversion + E4 Churn intelligence) — Inline "What's missing?" feedback widget on the upgrade-modal close action for free users. Today the upgrade modal opens at the 3-invoice limit and elsewhere, but every close-without-upgrade is a silent signal we never capture. Adds a small `<details>` element at the bottom of the modal: "Not ready? Tell us why →" → opens a 2-question form ("What's missing?" textarea + "What would change your mind?" textarea, both optional, max 500 chars each). Submission writes to a new `feedback_signals(id, user_id, source, missing, would_change_mind, created_at)` table. Distinct from #57 NPS micro-survey (different cohort — Pro users vs. free-cohort-considering-Pro), #73 cancel-reason survey (different funnel position — pre-conversion vs. cancelling), and #54 (different mechanism — exit-intent modal vs. embedded-in-existing-modal). MED churn intelligence — surfaces the actual conversion blockers Master can act on at the highest-intent moment in the funnel. ~80 lines + 1 schema migration + 1 route + 6 tests. Compounds with #46 (exit-intent modal) by capturing the same cohort earlier in their consideration.
- **#139** [GROWTH] [XS] (Epic E2 Trial → Paid Conversion) — Document.title escalation on day-1 trial. Today the page `<title>` reads "My Invoices · QuickInvoice" — same on day-1 as on day-7, no urgency signal in the browser tab strip. Add an Alpine `x-init`-driven prefix on dashboard render when `days_left_in_trial === 1`: `document.title = '⏱ Trial ends today — ' + document.title`. The browser tab strip is the freelancer's persistent context — when they have 12 tabs open and QuickInvoice is in the background, the urgency signal escalates from "easy to forget" to "visible in the tab title". Distinct from #95 (paid-invoice tab-flash — different trigger, different cohort, different message), distinct from the nav-pill #138 (different surface — tab title vs in-page nav). MED conversion via persistent ambient urgency. Pure JS, no server change. ~10 lines + 2 test assertions. Reverts cleanly on dismiss (the existing trial-banner dismiss handler can also restore the original title — minor extension).
- **#140** [GROWTH] [S] (Epic E4 Retention) — "Last paid: <client> — $<amount> on <date>" inline header line above the recent-revenue card on the dashboard. Single SQL extension to `db.getRecentRevenueStats`: same WHERE/JOIN, plus a sibling `SELECT client_name, total, paid_at FROM invoices WHERE user_id = $1 AND status='paid' AND paid_at IS NOT NULL ORDER BY paid_at DESC LIMIT 1` returned as `lastPaid: { clientName, total, paidAt }`. Threaded through `buildRecentRevenueCard()` and surfaced as a one-line header above the existing 3-tile grid. Pro/Agency only (matches the existing card visibility rule). MED retention via relationship-context surface — the freelancer's daily check-in now starts with a person, not a number. Distinct from #125 (top 5 clients widget — aggregate ranking vs single most-recent), distinct from #107 (raw stats — different signal: relationship vs aggregate volume). Compounds with #92 share-intent buttons (the user might reflexively re-share the next invoice to the same client). ~30 lines + 3 test assertions + 1 SQL extension.
- **#141** [GROWTH] [S] (Epic E3 Activation) — Dashboard "📨 Bill <last client> again" one-tap button next to the existing "+ New Invoice" CTA. Pre-fills the invoice form with the most-recent client's name + email + the most-recent invoice's line-items (zero-priced, ready for the freelancer to fill in the new amount). Reduces re-billing friction from "fill out 5 fields" to "edit 1 number + click send". Implementation: server-side query (same source as #140's last-paid lookup) → render a query-string deep-link `<a href="/invoices/new?duplicate_from=<invoice_id>&utm=dashboard-rebill">Bill <client> again</a>` on the dashboard header; route handler reads the param, hydrates the form with the duplicated invoice's client + line-items if the source invoice belongs to the user. Distinct from #27 (one-click invoice duplication — per-invoice surface, this is dashboard-level), distinct from #40 (full recurring-invoice cron — heavyweight scheduled vs. one-tap manual), distinct from #65 (template gallery — saved-template surface vs. last-client-shortcut). Activates the recurring-billing pattern for the 60% of freelancer SaaS users with ≤5 unique clients without committing to the full recurring-invoice feature. MED-HIGH activation (most-recent client is the most-likely-next-client by a wide margin). ~50 lines + 5 test assertions + 1 query-string-driven prefill path on /invoices/new.
- **#135** [GROWTH] [XS] (Epic E2 Trial → Paid Conversion) — Inline social-proof line on day-1 trial-urgent banner. Add a small grey line below the CTA: "Join 1,247 freelancers on Pro" (the `1,247` is the live `COUNT(*) FROM users WHERE plan IN ('pro', 'agency')` cached in module memory for 1 hour, with a static fallback "hundreds of freelancers" if the query fails). Compounds with #133 (price), #134 (time), and the existing urgency. Decision-moment social-proof reinforcement is the single most consistent conversion-lift pattern in SaaS A/B testing; pairing it with the urgency window (last day) is the sharpest application. Distinct from #20 social proof on landing/pricing (different surface — landing is pre-decision, this is at the conversion event), #112 live-counter on landing hero (different surface — visitor-bound, this is trial-user-bound), #98 testimonial wall (different surface). Pure read query with module-cache (same pattern as #112) + ~15 lines of view + 2 test assertions. MED conversion.
- **#136** [GROWTH] [S] (Epic E4 Retention) — First-paid-this-load CSS confetti micro-animation on the dashboard. When the user opens the dashboard and a `paid_at > last_dashboard_visit_at` invoice exists, fire a brief CSS-only confetti animation overlaying the recent-revenue card for ~2.5s. Uses the existing pattern from emoji-cannon CSS effects (no JS library required — pure `@keyframes` + `<span>` confetti pieces with randomized x-translates). Distinct from #95 tab-flash (different surface — background tab, this is foreground active session), and from the #30 cha-ching email (different channel — email, this is in-app). Together with #95 and #30, every paid event triggers three orthogonal "you got paid" signals across email, background tab, AND foreground session — the cha-ching feedback loop becomes inescapable across cohorts and contexts. New `users.last_dashboard_visit_at` column + middleware updating it on dashboard render. ~50 lines + 1 SQL column + 6 tests. MED retention via emotional spike. Compounds with #95 + #30 + the recent-revenue card cluster.
- **#137** [GROWTH] [S] (Epic E6 Distribution) — Free public tool at `/tools/invoice-template` — visitor enters business name + client name + 1-3 line items + total, gets a downloadable PDF blank invoice template with a small "Made with QuickInvoice — track payments → quickinvoice.app/auth/register?utm=template-tool" footer link. Pure no-signup flow → register-funnel feeder. Distinct from #103 late-fee-calculator (different tool, same channel — `/tools/`) and from #25 niche landing pages (different content type — interactive tool vs. SEO landing). Both `/tools/late-fee-calculator` and `/tools/invoice-template` together establish the "free tools" SEO surface that compounds: each tool ranks for high-intent "how do I [verb] X" searches that feed register-funnel directly. Implementation: re-use the existing PDF rendering path from `views/invoice-print.ejs` (already prints to PDF via browser); thin wrapper route generates a one-shot in-memory invoice (no DB persist — pure render pipeline). MED-HIGH SEO long-tail. ~80 lines + 5 tests + 1 new route. Compounds with #103.
- **#129** [GROWTH] [S] (Epic E4 Retention) — "🏆 Best ever" personal-record badge on the recent-revenue card. When the current window's `totalPaid` equals the all-time max `totalPaid` for any prior window of the same length, render a small gold pill "🏆 New record" next to the dollar tile. Single SQL CTE extension to `db.getRecentRevenueStats`: `MAX(totalPaid) OVER (PARTITION BY days_window) as historical_max`, compute by sliding the same window across the user's full invoice history. Distinct from #121 streak (consecutive weeks) and #123 vs-prior-period delta (forward-vs-backward — same direction, different framing) and from #107 raw stats (passive — record is achievement-framed). MED retention via Duolingo-style achievement reinforcement; "I just hit my best week" is the kind of moment users screenshot and share, compounding with #92 share-intent buttons (post-record reflexive share). ~40 lines + 1 SQL extension + 1 view block + 6 test assertions.
- **#130** [GROWTH] [S] (Epic E4 Retention) — Pull-to-refresh on mobile dashboard. Mobile users naturally pull-to-refresh when returning to "did anything land?" — today the dashboard doesn't intercept the gesture, so the user gets a full page reload (~600ms cold) instead of a JSON refresh of the just-shipped revenue card + invoice table top-rows (~80ms warm). Implement via a small Alpine touch-handler that detects `touchstart` near the top of scroll + `touchend` past a 60px-down threshold + intercepts the gesture; on success, fires the `/invoices/api/recent-revenue` endpoint already shipped for #117 + a new `/invoices/api/recent-list?limit=5` endpoint for the top invoices. Compounds with #95 (tab-flash on background paid notification — same daily-check-in cohort) and #122 (saved window — survives the pull-to-refresh). Distinct from #23 PWA install (different surface — native install vs. in-app gesture). MED retention on the mobile cohort (~40-50% of total traffic for freelancer SaaS). ~80 lines + 1 new API endpoint + 8 test assertions.
- **#123** [GROWTH] [XS] (Epic E3 Activation) — "vs prior period" delta badge on the recent-revenue card. Right of the dollar tile, render a small green/red badge: "↑ +42% vs prev 30d" / "↓ -8% vs prev 30d" / "—" (when prev period had $0 — division-by-zero guard). Single SQL extension to `db.getRecentRevenueStats`: window the prev period as `[NOW() - 2*days, NOW() - 1*days)` with a UNION ALL or a single CTE. Pure-fn helper computes delta + sign. MED retention via comparison-framing — the dollar-amount alone is data; the trend-arrow is meaning. Compounds with #117 (toggle): switching 7d/30d/90d also re-computes the delta badge. ~30 lines. Distinct from #107 (raw stats) and #88 (frequent-non-payer alert — different signal direction).
- **#124** [GROWTH] [XS] (Epic E3 Activation) — 3-day stale-draft yellow banner above the dashboard table. Today the user creates a draft, gets distracted, and the draft sits invisible inside the invoice table for weeks. A small yellow banner ("📝 You have 2 drafts from 3+ days ago — finish them?") appearing above the invoice table when `COUNT(*) WHERE status='draft' AND created_at < NOW() - INTERVAL '3 days' >= 1`. Click → scrolls + filters table to the draft cohort. Distinct from #83 (7+ day stale-draft, broader cohort). The 3-day cohort is the high-recovery-yield window — by 7 days the draft is mostly forgotten; by 3 days the freelancer still remembers what it was for. MED activation/revenue-recovery. ~20 lines + 1 db query.
- **#125** [GROWTH] [S] (Epic E4 Retention) — "Top 5 clients by revenue" dashboard widget. Renders below the recent-revenue card: a 5-row mini-table of `client_name | invoices_paid | total_paid` ordered by total_paid desc, computed over the same window as #117 selects (so 7d/30d/90d toggle drives both). Single SQL: `GROUP BY LOWER(COALESCE(client_email, client_name)) ORDER BY SUM(total) DESC LIMIT 5` with the same status='paid' filter. Pro/Agency only (free → null, like #107). MED retention via "who's keeping me afloat" awareness — surfaces the 80/20 client distribution that freelancers typically don't track. Compounds with #88 (frequent non-payer — opposite end of the same axis) and with #98 (review collection — top clients are the right testimonial sources). ~50 lines + 1 helper + 1 query. Distinct from #64 aging receivables (time-bucketed by overdue-ness vs. ranked by volume).
- **#126** [GROWTH] [S] (Epic E4 Retention) — Webhook retry queue with exponential backoff. Today `firePaidWebhook` is fire-and-forget — if the freelancer's Slack/Discord webhook 502s during a deploy or a transient network failure, the cha-ching message never lands. A small in-process retry queue with 3 attempts at 2s/8s/30s would close that gap. Implementation: wrap the existing `firePaidWebhook` to enqueue on non-2xx response; a single `setTimeout`-driven worker drains the queue. Persistence is optional (in-memory is fine — restart loses pending retries, but the cha-ching email already provides a backup notification path so the freelancer is never silently stranded). HIGH trust impact for Pro users who came for the Slack/Discord integration; once a single missed paid notification erodes trust the whole feature feels broken. ~80 lines + 6 tests. Distinct from #75 (which shipped the formatting; #126 is the reliability layer underneath).

  *(**#117 closed 2026-04-28 PM-3** — 7d/30d/90d window toggle on the recent-paid-revenue card shipped end-to-end. New `GET /invoices/api/recent-revenue?days=N` JSON endpoint (whitelisted `[7, 30, 90]`, falls back to 30, mounted before `/:id`); `views/dashboard.ejs` card wrapped in Alpine `recentRevenueCard()` scope with 3-button segmented control + `aria-pressed` + `:disabled="loading"` + `x-show="errored"` graceful-failure surface. Empty-window UX bug fixed mid-cycle: `card: null` from API now treated as $0/0/0 zeros, not an error. 25 new tests in `tests/recent-revenue-window-toggle.test.js` (13 API contract + 12 view render). Full suite: 47 files, 0 failures. See CHANGELOG.)*
- **#118** [GROWTH] [XS] — Stripe receipt URL on paid invoice view. Surface Stripe's auto-generated receipt URL (`payment_intent.charges.data[0].receipt_url`) as a "View Stripe receipt ↗" link in the action bar on paid invoices. AP-friendly artefact compounding. New `stripe_receipt_url TEXT` column (idempotent migration); webhook handler retrieves the PaymentIntent and persists the URL alongside the status flip. ~10 lines view change. Distinct from #70 receipt PDF (own-rendered) and #97 Stripe receipt-emails toggle (pre-pay).
- **#119** [GROWTH] [XS] — Inline "What's new" pulse-dot on nav bar (24-hr post-deploy attention-getter). Single `<span class="nav-dot animate-pulse">` next to "Settings", localStorage-gated on `lastChangelogSeen < CHANGELOG_VERSION`. MED retention via "the product is alive" signal. Pairs with #44 changelog widget but smaller. No DB.
- **#120** [GROWTH] [XS] — `<noscript>` SEO fallback hero on landing + niche pages. Hardens Lighthouse SEO score (CLS penalty on JS-rendered hero) and gives Google's first-pass indexer cleaner content. MED-HIGH SEO. ~10 lines on `views/index.ejs` + lp-niche partial.
- **#108** [S] — Side-by-side competitor pricing mini-table on `/pricing` page below the Free/Pro cards. Static rendered table: "QuickInvoice Pro $12/mo · FreshBooks Premium $33/mo · Bonsai Workflow $24/mo · HoneyBook Starter $19/mo · Wave (free; no email send + no payment links)". Bold the QuickInvoice row; cite "as of <month> 2026" with a small footnote. Distinct from #86 vs-pages (which are full SEO landing pages targeting `/vs/<competitor>` direct-traffic; this is the at-decision-moment comparison surface for users who arrive directly via /pricing). Distinct from #82 plan comparison table (which is Free-vs-Pro feature comparison; this is QuickInvoice-vs-competitor price comparison). MED conversion lift on the pricing-page-to-checkout step. Pure view change; one footnote pulls from a `data/competitor-pricing.json` fixture so updates land via single-file edit.
- **#109** [XS] — Sticky "+" floating action button (FAB) on mobile dashboard for "New invoice". Today the New-Invoice CTA is a top-right button that scrolls off-screen as the user scrolls a long invoice list (anyone with 30+ invoices loses the primary action). FAB pinned to the bottom-right (`fixed bottom-6 right-6 lg:hidden`) keeps the create-invoice loop one-tap-away on mobile. MED activation lift on the mobile cohort (which trends 40-50% of total traffic for freelancer SaaS). Distinct from #91 dashboard "Copy pay link" button (different action, different row-scope). Distinct from #23 PWA manifest (different surface — install-time vs in-app). Pure view change; conditional render via Tailwind's `lg:hidden` + a single anchor tag. ~10 lines.
- **#110** [M] — Passwordless magic-link login at `/auth/magic-link`. POST email → server sends a one-time-token URL via Resend → click → session created. Reduces login friction (the 2nd most-bounced auth surface after register, where bcrypt-cost-12 makes wrong-password retries painful). Different shape from #17 Google OAuth (which has a hard fork: do you have a Google account?), different surface from #13 (closed) auto-login on register. MED conversion at the login step + MED retention via "I never have to remember my password". Gated on Resend in prod (joins #11/#12/#66/#71/#77/#80/#90 in the Resend-block). Implementation: add `magic_token`/`magic_token_expires_at` columns on users; new `routes/auth.js GET/POST /magic-link`; new `views/auth/magic-link-sent.ejs` view; reuses `lib/email.js sendEmail` infrastructure.
- **#111** [S] — "Show client preview" pre-send modal on `views/invoice-form.ejs`. Adds a "Preview as client" button next to the existing Save/Send buttons that opens the public read-only invoice URL (#43) inside an iframe modal — freelancer can see exactly what their client will see before hitting Send. Pre-send confidence lever — addresses the very common "is this going to look right?" friction that drives users to send a test invoice to themselves first. Distinct from the existing PDF preview path (different artefact: client-facing web page vs PDF). Gated on #43 public read-only invoice URL landing first (1-week sequencing, not a hard block). MED activation. Pure view change once #43 is live; ~30 lines for the iframe-modal Alpine component.
- **#96** [XS] — "📩 Send a copy to me too" checkbox on invoice send (all tiers; sticky default after first toggle) (MED activation/trust; distinct from #71 always-on Pro auto-BCC and from #84 plain-language client body — this is a per-invoice freelancer CC that closes the "did the email actually go out?" trust gap for first-time senders. Pure form change + one boolean column on `invoices` or per-user setting; reuses existing `sendInvoiceEmail` flow with an extra `cc` param.)
- **#97** [XS] — Stripe Receipt-emails default-on toggle in settings (MED conversion-and-retention via "Stripe sent the client a professional receipt automatically — zero extra work for the freelancer" — Stripe natively sends payment-receipt emails when `receipt_email` is set on the Payment Link, but today we leave it unset so the freelancer is missing a free Stripe-sent professionalism signal. Toggle defaults ON for new Pro users; setting flows into `createInvoicePaymentLink` as `receipt_email: invoice.client_email`. Distinct from #55 auto thank-you (different sender — Stripe vs QuickInvoice — and different content), distinct from #61 attach-PDF-on-send (different surface — pre-payment vs post-payment), distinct from #70 receipt PDF (different artefact — Stripe email vs QuickInvoice-rendered PDF).)
  *(**#92 closed 2026-04-27 PM-5** — three share-intent buttons (WhatsApp / SMS / Email) shipped on the invoice-view Payment Link card (Pro users only — same gate as Copy / Preview). Pure-anchor implementation: `https://wa.me/?text=<encoded>` (target=_blank rel=noopener), `sms:?&body=<encoded>` (cross-platform-tolerant variant — iOS + Android Messages), and `mailto:<encoded-client_email>?subject=…&body=…` with graceful degradation to `mailto:?subject=…` when client_email is empty. Prefilled message body single source of truth across all 3 buttons, total formatted as `$N.NN`, greeting falls back from `"Hi <name>,"` → `"Hi,"` when client_name empty (no dangling `"Hi ,"`). Health Monitor pass (same cycle) added defence-in-depth: `client_email` is now percent-encoded before being concatenated into the mailto recipient slot, closing a `?cc=attacker@evil.com` injection vector that a hostile (DB-stored or session-attacker) email value could otherwise have triggered. 13 new tests in `tests/share-intent-buttons.test.js` (12 base + 1 mailto-injection regression test), all passing. Full suite still green: 41 test files, 0 failures.)*
- **#95** [XS] — In-page tab-title flash + favicon dot on the dashboard when a paid invoice lands while the tab is in background (MED retention/cha-ching-stickiness; pairs with #30 instant paid-notification email — same trigger, different surface, recovers attention without asking for browser-push permission; pure JS, no new server endpoint required if we re-poll the existing dashboard data on a 30s interval)
  *(**#101 closed 2026-04-28 AM** — inline annual-savings $-amount pill shipped on `views/pricing.ejs` + `views/settings.ejs` + `views/partials/upgrade-modal.ejs`. Three reactive Alpine surfaces now render a "Save $45/year vs. paying monthly" green-pill callout when `cycle === 'annual'`. **pricing.ejs:** the existing small "Save $45/year vs. monthly" gray text was promoted to a prominent emerald-400 inline pill rendered next to the `$99/yr` price (within the same flex row, gap-2) — high-prominence position; below the price, the supporting text was rewritten to a concrete-equivalence framing ("That's 3 months free vs. paying monthly.") for additional conviction. **settings.ejs:** added a brand-new green pill (`bg-green-100 text-green-700` + 💰 emoji + rounded-full) below the Monthly/Annual toggle row, only visible when annual is selected — closes the gap where free users on the settings page saw only the percentage but not the dollar figure. **upgrade-modal.ejs:** also added the new green pill, repositioned the toggle buttons to also display the price ("Monthly — $12/mo" / "Annual — $99/yr" — was previously bare "Monthly" / "Annual"), and removed the redundant fine-print "(save $45/year)" parenthetical at the bottom of the modal since the pill above it is now the canonical surface for the savings number. All three views x-cloak the annual-only pill to prevent flash-of-unselected on first render. No DB/backend touch — pure view change. The existing 9 `tests/annual-billing.test.js` assertions all pass unmodified (they assert the toggle structure + hidden `billing_cycle` input; the new pill sits inside the same Alpine scope). 4 new pill-presence assertions added in this cycle's Test Examiner pass (see CHANGELOG). MED conversion lift on the price-page-to-checkout step — every previously-unpriced annual-toggle surface now visually emphasises the dollar savings at the moment of decision.)*
- **#102** [XS] — Persist per-user `timezone` on signup + use it for due-date rendering and reminder send-time. Today `views/dashboard.ejs` and `views/invoice-view.ejs` both call `toLocaleDateString('en-US', ...)` with no `timeZone` option — server-local time leaks into client UI, and the daily reminder cron (`jobs/reminders.js`) fires at server-local midnight which lands at 4-7am for European Pro users. MED retention via "the app gets it right for ME". Implementation: capture `Intl.DateTimeFormat().resolvedOptions().timeZone` on the registration form via a hidden input, store in a new `users.timezone` column, default `'UTC'`, surface as a settings dropdown for edit. Reminder cron stays daily at server time but skips users whose local time hasn't passed 9am yet (one extra column predicate). Distinct from #99 multi-language PDF (timezone vs language are orthogonal axes of localisation).
- **#112** [XS] — Live "Trusted by N freelancers in M countries" social-proof counter on `views/index.ejs` hero section. Reads `SELECT COUNT(*) FROM users` and `SELECT COUNT(DISTINCT business_country) FROM users` (the country column lands with #102 timezone or sooner). Cache the result in module-scoped memory for 1 hour to avoid hitting the DB on every landing-page render. Static fallback strings ("hundreds of freelancers" / "30+ countries") if the query fails — landing-page render must never be brittle. Distinct from #20 social proof testimonials (testimonial wall is qualitative; this is quantitative + dynamic + impossible-to-fake-feeling). Distinct from #36 OG metadata (different surface — meta tags vs visible hero). MED conversion via dynamic vs. static social proof; the live-counter format ("Joined by 1,247 freelancers across 42 countries") feels current and growing in a way no static "Loved by freelancers" headline does. Pure read query + 1 module-cache + ~10 lines of view. Gated only on having ≥100 users — ship the code now, flip the wrapper on once we cross the threshold.
- **#113** [XS] — Per-niche-page custom `<meta name="description">` blurb. Today every niche page in `views/partials/lp-niche.ejs` shares the same description (or none — lookup confirms). Google's SERP snippet is the single most important conversion lever for organic search, and unique per-page descriptions lift CTR 5-15% over duplicates. Implementation: add a `description` field to each entry in the niche-pages registry (`routes/landing.js#listNiches()`); thread it through the render call to `views/partials/head.ejs` which already has the `<meta name="description">` slot. ~10 lines + 6 niche-specific blurbs (designer, developer, writer, photographer, consultant, plus any added since). Distinct from #36 OG/Twitter Card metadata (different cohort — paid social shares vs. organic-search SERPs). MED SEO snippet-CTR lift. Compounds with #25 expand-niches and #103 late-fee-calculator tool — every new SEO surface gets unique descriptions from day one once this lands.
- **#116** [XS] — Empty-state "Watch a 30-second demo" expandable accordion on the dashboard. Today the empty state in `views/dashboard.ejs` shows the onboarding step list — useful for "I'm here to do it myself" users, but provides no passive-explanation surface for "show me first" users. Add a single `<details>` element with `<summary>👉 See how QuickInvoice works in 30 seconds</summary>` containing a 4-frame autoplay GIF or a `<video>` tag pointing at `public/demo.gif|.mp4`. Distinct from #60 demo-mode (which is a full interactive sandbox at `/demo`, requires no signup, M complexity); this is the pre-existing-signup empty-state explainer that closes the "I made an account but I don't know what to do" gap. Pre-existing onboarding step list stays — accordion sits above it as a 1-click optional alternative. MED activation lift on the empty-state cohort. Pure view change + 1 video asset (the asset is a TODO_MASTER deliverable but the placeholder + structural layout ships now).
...
- **#98** [S] — Public review/testimonial collection page `/r/<user-slug>` for Pro users (MED-HIGH virality + retention compounding; every paid client is a potential review source. After paid status, the auto thank-you email (post #55) appends a "Mind leaving a 30-second testimonial?" link → public review wall; reviews are stamped on the freelancer profile (post #78) and on `/r/<slug>`. Distinct from #20 social proof (uses QuickInvoice's collected reviews on landing — not the user's). Distinct from #57 NPS micro-survey (NPS is internal churn signal, not external trust artefact). Distinct from #78 (#78 is the profile page surfaces "latest unpaid invoice"; #98 surfaces collected reviews).)
- **#99** [S] — Multi-language invoice PDF rendering (initial: ES / FR / PT / DE) (MED activation expansion in non-EN markets — LATAM/IBERIAN/DACH freelancer market is underserved by US-first invoicing tools. Add a `language` column to `invoices` (per-invoice override of `users.default_language`); translate the ~15 static labels in `views/invoice-print.ejs` (Invoice/Subtotal/Tax/Total/Due Date/Bill To/Thank you/etc) via a small `lib/i18n.js` helper. Distinct from #24 multi-currency — currency and language are orthogonal concerns. Pairs with #25 niche landing pages — could spawn `/es/freelance-disenadores`, `/pt/freelance-tradutores` etc, expanding TAM beyond the US-anglophone segment.)
- **#100** [S] — Stripe Climate / charity round-up toggle on invoice pay links (Pro feature) (MED retention via emotional/values-aligned moat; particularly resonant for freelancers in design / creative niches who already advertise eco-credentials. 1% of each successful payment is donated automatically to a Stripe Climate-listed climate fund or to one of 3 partner charities (user-selectable in settings). Distinct from #67 tip toggle (different recipient — charity vs freelancer). Distinct from #79 BNPL (different `payment_method_types` family). Adds a "carbon-neutral invoicing" landing-page bullet that's defensible vs FreshBooks/Wave/Bonsai. Implementation: pre-create a Stripe `Tax Rate` or `Coupon` representing the 1% donation; post-pay, fire a Stripe `Transfer` from connected funds to a charity-controlled Stripe account. May require Master action to create the partner-charity Stripe accounts — flagged but not blocking the code work.)
...
**Income-critical [GROWTH] — S complexity (continued)**
- **#103** [S] — Free public tool at `/tools/late-fee-calculator` — interactive calculator (amount + days overdue + jurisdiction-default annual rate) outputs the legally-defensible late fee. Embeds a single "Send this invoice with auto-reminders → no late fees needed" CTA at the bottom that lands on `/auth/register?utm=late-fee-tool`. MED-HIGH SEO long-tail (`how much late fee can I charge`, `freelance late fee calculator`, `invoice past due fee`) — high-intent searches with intent-aligned conversion. Distinct from #25 niche landing pages (uses customer-segment angle), #86 vs-pages (uses competitor angle), #46 exit-intent modal (uses bounce-cohort recovery angle). Compounds with #22 Late Fee Automation Pro feature — the free tool seeds the search, the Pro feature collects on it.
- **#104** [S] — Browser extension MVP (Chrome Web Store) — lightweight extension that detects `https://buy.stripe.com/...` Pay Link URLs in the user's open Gmail/Outlook tabs and overlays a 1-click "Mark as paid in QuickInvoice" badge. Calls a new `POST /api/v1/invoices/mark-paid-by-payment-link` endpoint (which already exists for the Stripe webhook — re-purposable as a plain authenticated endpoint). Distinct from #69 embeddable JS widget (lives on the freelancer's own site) and #23 PWA (mobile-install vs desktop browser). New distribution surface — the Chrome Web Store has 2.5B users and is a discovery channel adjacent to but disjoint from search. Income relevance: MED retention via daily-touchpoint feature + virality (extension users are evangelists). Gated on #32 API Key Auth + REST Endpoints landing first (one-week sequencing dependency, not a hard block).
- **#114** [S] — Minimum-viable referral attribution: `?ref=<user_slug>` query-string capture at register. When a freelancer shares any QuickInvoice URL with a peer, the peer arrives carrying `?ref=jbensen42` — server reads it on `GET /auth/register`, drops a 30-day signed cookie, writes `users.referrer_id` on signup. Dashboard then renders a one-time "💸 You're a referral! @jbensen sent you here — they get a free Pro month if you upgrade" callout. Distinct from #18 (full Stripe-coupon-driven referral program — M complexity, requires a Stripe coupon flow + payouts + email triggers); this is the attribution-only sub-step that unlocks the data layer for #18 later, AND captures word-of-mouth signups happening today that we currently lose attribution on. MED-HIGH virality lift — the existence of "your friend gets credit" alone is a known meaningful share-incentive even before the coupon ships. Implementation: add `users.referrer_id INTEGER NULL` column (FK to users.id, idempotent migration); add the cookie+register hooks in `routes/auth.js`; add the dashboard callout block. Pure attribution, ~50 lines + 1 schema column. ~2 hrs.
- **#115** [S] — "Reply to client" mailto chip on `views/invoice-view.ejs` for paid invoices. Renders next to the existing share-intent buttons, only visible when `invoice.status === 'paid'` and `invoice.client_email` is set. Opens `mailto:client@example.com?subject=Thanks for paying invoice INV-001&body=Hi <name>,%0A%0AThanks for the prompt payment on invoice <number>. Looking forward to working together again.%0A%0A— <freelancer_name>`. Closes the relationship-management loop after a paid invoice — every paid client is a potential repeat-business node, and a 30-second thank-you note dramatically lifts the rebook rate. Distinct from #55 auto thank-you email (different sender — Stripe-receipt automation vs. manual freelancer-as-human follow-up; the personal note is the conversion-to-rebook lever, not the receipt). Distinct from #92 share-intent buttons (different lifecycle moment — pre-pay share vs. post-pay relationship). MED retention via increased rebook rate. Pure view change, ~20 lines, percent-encoded body to prevent injection (same defence-in-depth pattern as #92 mailto recipient).
- **#121** [GROWTH] [S] — Dashboard "Streak" gamification badge — "X weeks in a row sending invoices" / "Y weeks in a row collecting paid invoices". Renders as a small badge in the dashboard header next to the "+ New Invoice" button. Computes from invoices: `MAX(consecutive_weeks WHERE status='paid')`. Duolingo/Strava cadence-reinforcement loop — even a 2-week streak triggers a "🔥 2 weeks straight" affordance. Distinct from #107 (dollar/count) and #44 changelog (product-news). MED retention. SQL is one CTE (`generate_series` weeks + LEFT JOIN paid invoices); helper + view block are ~50 lines.

**Income-critical [GROWTH] — M / L (larger; plan deliberately)**
- **#105** [M] — Multi-business profiles per user — agencies and dual-brand consultants frequently invoice under 2-3 distinct business names. Today `users.business_name` / `business_address` / `business_phone` / `business_email` are single rows on `users`. Add a `business_profiles` table (FK to user, plus the four fields), default-profile fallback, and a profile-picker dropdown on the invoice form. HIGH retention for power users (the freelancer who's "the agency" of one + the contractor under their own name + the consultant under a project-specific brand — all common patterns). Distinct from #9 InvoiceFlow Team Seats (different users sharing one account) and from #54 deposits (different feature). Schema migration is the heaviest part — bundles cleanly with H8 composite-index migration if the next deploy includes both.
...
### #91. [DONE 2026-04-30] [GROWTH] Dashboard "Copy pay link" icon button per row (Pro/Agency only) [XS]

**App:** QuickInvoice (Node.js)
**Impact:** MED activation lift — single most-frequent post-send action for a Pro freelancer (paste pay link into Slack/email/SMS to chase a slow client). Cuts the share-link-with-client flow from 3 clicks (open invoice → click Payment Link card → click Copy) to 1 click straight from the dashboard table. Compounds with #30 paid-notification email + #75 Slack/Discord webhook (post-pay momentum) on the recovery side.
**Effort:** XS
**Prerequisites:** None (data already on `getInvoicesByUser` SELECT *).

**What shipped (2026-04-30):**
1. `views/dashboard.ejs` — conditional 6th column for `user.plan === 'pro' || 'agency'`. Per-row Alpine `x-data="{ copied: false }"` scope wrapping a real `<button type="button">` with SVG copy icon + "Copy link" label. `@click.stop` (Alpine modifier) + wrapping `<td onclick="event.stopPropagation()">` defence-in-depth so row navigation never fires on button click. Successful copy → green ✓ + "Copied" affordance for 2s. Both column header + cell carry `print:hidden`. Rows without `payment_link_url` (drafts) render an empty cell (preserves table layout) but don't render the button.
2. `tests/dashboard-copy-pay-link.test.js` — 11 tests, all passing. Hard-gates the column to Pro+Agency (asserts 5-vs-6 column count for free/Pro), asserts independent Alpine scopes per row, both propagation-stop layers, print:hidden, real-button accessibility with reactive aria-label, empty-state branch.
3. `package.json` — test runner extended.
