...
> **Audited:** 2026-04-28 PM-2 (Task Optimizer cycle, 17th pass). This cycle's deltas: (a) **#107 closed** — last-30-day paid-revenue stats card shipped on `views/dashboard.ejs` above the existing 3-card all-time grid. New `db.getRecentRevenueStats(userId, days=30)` runs a single SELECT (`SUM(total) + COUNT(*) + COUNT(DISTINCT client)` filtered to `status='paid' AND updated_at >= NOW() - 30d`); `updated_at` serves as the paid-time proxy. New pure helper `buildRecentRevenueCard(user, stats)` in `routes/invoices.js` decides render (free → null, no paid → null). Card is emerald-50/200, `print:hidden`, parallel-fetched alongside dashboard queries. 24 new tests in `tests/recent-revenue-stats.test.js` (10 pure-logic + 9 render + 5 SQL-contract assertions added in Role 2's pass). Full suite: 46 files, 0 failures. (b) **5 new [GROWTH] items (#117-#121)** from this cycle's Growth Strategist pass: #117 7d/30d/90d window toggle on the new revenue card [XS, MED retention via user-controllable timeframe]; #118 Stripe-receipt-URL on paid invoice view [XS, MED retention via accountant-friendly artefact]; #119 nav-bar "What's new" pulse-dot [XS, MED retention]; #120 `<noscript>` SEO hero fallback [XS, MED-HIGH SEO via Lighthouse-score lift]; #121 dashboard "Streak" gamification badge [S, MED retention via Duolingo/Strava cadence-reinforcement]. (c) **1 new [MARKETING] item in TODO_MASTER (#59)** — co-marketing partnership outreach to Calendly / Bonsai / Notion / Plaid+Mercury+Wise — partner-channel distribution (~10 hrs initial outreach, est. 24-480 paid signups/yr across 4 active partnerships). (d) **UX direct fixes (2 items)** — `views/dashboard.ejs` revenue-card "Paid" tile label → "Revenue" (kills word-collision with neighbouring "Clients paid you" / "Invoices paid"); added conditional "ALL-TIME TOTALS" subtitle above the existing 3-card stats row (only when the new 30-day card renders) — creates clean temporal contrast between recent and lifetime stats.
>
> Cross-checks for non-overlap on this cycle's adds: #117 vs #107 (#117 is the user-controllable timeframe surface for the card #107 just shipped — different surface, same query helper; gated on #107 landing first which it just did); #118 vs #70 vs #97 (3 distinct receipt artefacts: Stripe-hosted URL post-pay vs own-rendered PDF vs pre-pay receipt-email toggle); #119 vs #44 (#119 is the attention-getter dot; #44 is the widget itself); #120 vs #56 vs #36 (3 distinct SEO surfaces: noscript fallback hero vs robots/canonical headers vs OG metadata); #121 vs #107 vs #44 (streak-cadence is orthogonal to dollar metrics + product-news). MARKETING #59 vs #43 listicles (third-party editorial), vs #58 SaaS comparison directories (passive submission), vs #44 LinkedIn outreach (direct-to-end-user) — partner-channel distribution is a distinct un-mined acquisition lever. TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL) all remain genuinely open. No items flip to [LIKELY DONE - verify] this cycle.
>
...
- **#117** [GROWTH] [XS] — 7d / 30d / 90d window toggle on the new recent-paid-revenue card (#107). Today the card is hard-pinned to a 30-day window. Small Alpine `x-data="{ window: '30d' }"` toggle row above the 3 tiles re-fetches via a tiny `GET /api/dashboard/recent-revenue?days=N` JSON endpoint and re-renders the tiles client-side. 7d → "is this week working?"; 90d → quarterly-feel. MED retention. Reuses the existing helper that already clamps the days arg to [1, 365]. ~30 lines + 1 JSON route.
