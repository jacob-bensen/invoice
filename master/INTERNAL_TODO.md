...
> **Audited:** 2026-04-28 PM-3 (Task Optimizer cycle, 18th pass). This cycle's deltas: (a) **#117 closed** — 7d/30d/90d window toggle on the recent-paid-revenue card shipped end-to-end. New `GET /invoices/api/recent-revenue?days=N` JSON endpoint (whitelisted `[7, 30, 90]`, falls back to 30) reuses `db.getRecentRevenueStats` + `buildRecentRevenueCard`. Route mounted before `/:id` so `'api'` isn't matched as an invoice id; `Cache-Control: no-store` keeps toggle data fresh. `views/dashboard.ejs` recent-revenue card wrapped in `x-data="recentRevenueCard(initial)"` Alpine scope with 3-button segmented control (`role="group"`, `:aria-pressed`, `:disabled="loading"`), reactive `x-text` on tiles for re-render, `x-show="errored"` graceful-failure line. Empty-window UX bug fixed mid-cycle: `card: null` from API now treated as "$0 / 0 / 0 for this window" instead of an error (Role 5's Auditor pass). 25 new tests in `tests/recent-revenue-window-toggle.test.js` (13 API contract + 12 view render). Full suite: 47 files, 0 failures. (b) **5 new [GROWTH] items (#122-#126)** from Growth Strategist pass: #122 [XS] persist last-selected window in `localStorage` (E3, MED retention); #123 [XS] "vs prior period" delta badge on the revenue card (E3, MED retention via comparison framing); #124 [XS] 3-day stale-draft yellow banner above dashboard table (E3, MED activation, distinct from #83's 7-day cohort); #125 [S] "Top 5 clients by revenue" widget (E4, MED retention via 80/20 client awareness); #126 [S] webhook retry queue with exponential backoff (E4, HIGH trust impact for Slack/Discord users). (c) **1 new [MARKETING] item in TODO_MASTER (#61)** — affiliate program setup via Rewardful with 25%-recurring 12-month commission targeting freelance YouTubers / newsletter authors. Distinct from #18 referral (peer-to-peer) and #59 co-marketing (partner integrations). Conservative ramp: 10-50 paid signups/month at month 6 → $90-$1350/month net MRR-equivalent after commission. (d) **Bootstrap delta — APP_SPEC.md re-synced**: prior spec only described the InvoiceFlow Java/Spring app; reality is QuickInvoice (Node.js) is primary. New spec documents both apps as one file. Master action #60 [SPEC-REVIEW] filed for human verification. (e) **EPICS.md created** — 9 epics extracted from existing INTERNAL_TODO clusters (E1 Stripe Payments [COMPLETE]; E2/E3/E4 active; E5/E6/E7/E8 planned; E9 paused on Resend gate). 3-active-epic budget held: E2 Trial→Paid, E3 Activation, E4 Retention. (f) **UX direct fix (1 item)** — `views/dashboard.ejs` toggle empty-window handling now shows truthful zeros instead of erroring on the "you weren't paid this week" cohort.
>
> Cross-checks for non-overlap on this cycle's adds: #122 vs #117 vs #102 (UI memory vs UI toggle vs DB-persisted timezone); #123 vs #107 vs #88 (delta vs raw vs categorical alert — three different signal types); #124 vs #83 (3-day vs 7-day cohorts of the same task type, both surfaces co-exist); #125 vs #64 vs #88 (top-5 by revenue vs aging by overdue-ness vs categorical non-payer alert — three orthogonal client analytics); #126 vs #75 (reliability layer vs formatting layer at the same surface). MARKETING #61 vs #18 vs #59 (creators with audiences vs end-user peer referrals vs partner integrations — three distinct distribution channels). TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL), #60 (SPEC-REVIEW from this cycle's bootstrap), #61 (affiliate program from this cycle) all open. No items flip to [LIKELY DONE - verify] this cycle.
>
...
- **#122** [GROWTH] [XS] (Epic E3 Activation) — Persist the user's last-selected recent-revenue window in `localStorage`. Today every dashboard load resets the toggle to 30d; a Pro user who prefers 7d for daily check-ins re-clicks every visit. Read on Alpine `init()` from `localStorage.getItem('qi.recentRevenueDays')`, fall back to 30 if missing/corrupt; on `select(window)`, write back. ~10 lines inside the existing `recentRevenueCard()` Alpine factory in `views/dashboard.ejs`. MED retention via "the app remembers me". No DB, no migration, no API change. Distinct from #117 (which shipped the toggle UI) and from #102 (per-user timezone — DB-persisted user preference vs. per-device localStorage UX preference).
