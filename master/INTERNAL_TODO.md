...
> **Audited:** 2026-04-28 PM-5 (Task Optimizer cycle, 20th pass). This cycle's deltas: (a) **#127 closed** — quiet-window recovery CTA on the recent-revenue card. New `unpaidCount` field on `db.getRecentRevenueStats` (single SQL round-trip — refactored to `WHERE user_id` + `FILTER` aggregates so paid-window stats AND not-windowed unpaid count come back together; net-equal cost vs. previous query). Threaded through `buildRecentRevenueCard()` and surfaced at the top level of `GET /invoices/api/recent-revenue` so the CTA fires even when `card===null` (zero paid in window). `views/dashboard.ejs` Alpine scope tracks `unpaidCount`; new reactive `x-show="totalPaid === 0 && unpaidCount > 0"` block renders an inline anchor "📨 Send N follow-up(s) now →" deep-linking to `#invoices-table` (new id on the existing table wrapper). Plural `s` gated by `x-show="unpaidCount !== 1"` so 1 follow-up doesn't render "Send 1 follow-ups". 24 new tests in `tests/recent-revenue-quiet-window-cta.test.js` (5 SQL contract + 5 helper threading + 4 API contract + 11 view render). Full suite: 49 files, 0 failures. (b) **3 new [GROWTH] items (#131-#133)** from Growth Strategist pass: #131 [XS] truly-quiet-window CTA variant (totalPaid===0 + unpaidCount===0 → "Send a new invoice" prompt — sibling block to #127, completing the empty-state action coverage) (E3, MED activation); #132 [XS] mobile haptic toggle feedback via `navigator.vibrate(8)` (E4, LOW-MED mobile retention); #133 [XS] inline annual-savings pill on day-1 trial-urgent banner (E2, MED conversion at trial-end moment). (c) **1 new [MARKETING] item in TODO_MASTER (#63)** — G2 quarterly award category submissions (Best Free Invoice Software, Best Small-Business Billing Software, Easiest to Use, Best Estimated ROI). Free; ~1 hr initial + ~30 min/quarter ongoing for evergreen badge + press-release secondary distribution. Distinct from #26/#49 (continuous profile + reviews), #61 (affiliate), #62 (creator outreach), #59 (co-marketing). The editorial-recognition acquisition layer. (d) **UX direct fix (1 item)** — `views/dashboard.ejs` quiet-window CTA `<p>` now carries `role="status"` + `aria-live="polite"` so screen readers announce the action option when it appears reactively after a toggle. Regression-guard test added.
>
> Cross-checks for non-overlap on this cycle's adds: #131 vs #127 vs #124 vs #95 (truly-empty-window prompt vs recovery prompt vs stale-draft prompt vs paid-event reward — four orthogonal state-driven CTAs); #132 vs #117 vs #122 vs #128 vs #130 (haptic vs visual click vs per-device memory vs keyboard vs pull-to-refresh — five complementary input modalities on same surface); #133 vs #45 vs #101 vs #46 vs #47 (trial-urgent banner pricing pill vs banner-base copy vs already-shipped pricing/settings/upgrade-modal pill vs exit-intent vs monthly→annual prompt — five orthogonal pricing-reinforcement surfaces). MARKETING #63 vs #26/#49 vs #61 vs #62 vs #59 vs #58 (G2 editorial recognition vs continuous profile + reviews vs always-on affiliate vs one-time creator outreach vs partner co-marketing vs passive directory listings — six distinct distribution motions). TODO_MASTER reviewed: #18 (Resend API key), #38 (OG image asset), #39 (APP_URL), #60 (SPEC-REVIEW), #61 (affiliate program), #62 (creator outreach), #63 (G2 awards from this cycle) all open. No items flip to [LIKELY DONE - verify] this cycle.
>
...
## OPEN TASK INDEX (priority order, post-2026-04-28 PM-2 audit — 17th pass)
...
- **#133** [GROWTH] [XS] (Epic E2 Trial → Paid Conversion) — Inline annual-savings pill on the day-1 trial-urgent banner (#45). Today the day-1 banner reads "Last day of your Pro trial — add a card before midnight..." with a single CTA. Expanding the copy to "Last day — lock in $99/year (3 months free)" before the CTA makes the conversion-moment pricing decision explicit at the highest-leverage moment. Distinct from #46 pricing-page exit-intent (different surface), #47 monthly→annual upgrade prompt (different cohort — already-paying users), #101 already-shipped annual-savings pill on /pricing + /settings + upgrade-modal (different surface — the trial banner is the most-attended trial-end touchpoint and currently has no price reinforcement). Pure copy edit + the existing pill-component pattern from #101. ~10 lines on the existing `trial-urgent` branch in `views/dashboard.ejs` + 2 test assertions for the new copy + the pill structure. MED conversion at the trial-end moment.
