...
// Pattern-allow for specific-invoice deep-links: /invoices/<positive integer>.
// Triggered emails that target one row (stale-draft) bake this into the CTA so
// a user clicking 24h+ later lands signed-in on the exact draft instead of
// /dashboard. Strictly digit-only with no leading zero, no negative, no
// decimal, no trailing segment — so this cannot be widened to
// /invoices/<id>/edit, /invoices/<id>foo, /invoices/-1, /invoices/0, etc.
// Ownership is enforced downstream by `GET /invoices/:id`, so the worst-case
// of a guessed id is the same 404/403 the user would see when signed in
// normally — there is no additional surface opened by adding this pattern.
const INVOICE_ID_PATTERN = /^\/invoices\/[1-9][0-9]*$/;
...
function safeNextPath(raw) {
  ...
  if (!trimmed.startsWith('/')) return null;
  if (NEXT_ALLOW_LIST.has(trimmed)) return trimmed;
  if (INVOICE_ID_PATTERN.test(trimmed)) return trimmed;
  return null;
}
