...
/*
 * The optional `unsubscribeUrl` switches a send from transactional-only
 * into "lifecycle email" mode: a visible footer is appended to both
 * html and text bodies, and RFC 8058 `List-Unsubscribe` +
 * `List-Unsubscribe-Post: List-Unsubscribe=One-Click` headers are
 * emitted so Gmail/Apple Mail/Outlook can offer their native one-tap
 * "Unsubscribe" button. Callers that pre-build their own `headers`
 * still win — explicit headers take precedence per-key over the
 * auto-injected pair.
 */
async function sendEmail({
  to, subject, html, text, replyTo, from, bcc,
  ...
  if (unsubscribeUrl) {
    const withFooter = appendUnsubscribeFooter(finalHtml || '', finalText || '', unsubscribeUrl);
    finalHtml = withFooter.html || undefined;
    finalText = withFooter.text || undefined;
  }

  const mergedHeaders = Object.assign({}, unsubscribeHeaders(unsubscribeUrl), headers || {});
...
/*
 * Owner-side BCC on outbound client invoice emails (Milestone 3 — first
 * invoice created → first invoice sent). When the user opts in via
 * settings, every invoice the platform sends to their client also
 * delivers a silent copy to the freelancer's own inbox. Two activation
 * levers in one toggle:
 *   - Pre-send anxiety ("did it actually go out?"): the freelancer sees
 *     the delivered copy in their inbox the moment the client does, so
 *     "send by email" stops being a black box.
 *   - Post-send paper trail: the BCC sits in their Sent-adjacent folder
 *     so they can forward, re-send manually, or scan when a client
 *     replies asking "what was the amount again?".
 *
 * Returns null when the toggle is off, the owner has no resolvable
 * address, OR the resolved owner address equals the invoice's
 * client_email (sending to the same address twice via to+bcc duplicates
 * the inbox and confuses the freelancer about whether the client
 * received it). Case-insensitive equality check on the dedupe so
 * `Me@x.com` doesn't slip through `me@x.com`.
 */
function resolveOwnerBcc(invoice, owner) {
  if (!owner || !owner.bcc_invoice_emails) return null;
  const addr = (owner.email || '').trim();
  if (!addr) return null;
  const clientEmail = (invoice && invoice.client_email || '').trim();
  if (clientEmail && clientEmail.toLowerCase() === addr.toLowerCase()) return null;
  return addr;
}
...
async function sendInvoiceEmail(invoice, owner) {
  ...
  return sendEmail({
    ...
    bcc: resolveOwnerBcc(invoice, owner)
  });
}

/*
 * Test-send (Milestone 3 — first invoice created → first invoice sent).
 * Delivers the EXACT same subject + html + text body the client would
 * receive, but to the freelancer's own inbox so they can preview the
 * actual mail-client rendering (Gmail / Apple Mail / Outlook) before
 * tapping "send to client". Removes the dominant pre-send anxiety beat
 * ("I don't know what this looks like to them") that keeps users stuck
 * in draft after the static iframe preview at /invoices/:id/preview-email.
 *
 * Recipient is hardcoded to `owner.email` (never reads invoice.client_email)
 * so there is no spam vector: an authenticated user can only test-send to
 * their own registered address. Subject is prefixed with `[Test] ` so the
 * freelancer's inbox search can distinguish a preview from a real outbound
 * copy of the same invoice. No List-Unsubscribe header — this is a
 * transactional self-directed send, not a lifecycle email; CAN-SPAM
 * commercial-email rules apply to recipients other than the sender.
 *
 * Soft-fails on every error path (missing owner email, RESEND_API_KEY unset,
 * Resend throw) so the caller never throws — matches the sendInvoiceEmail
 * contract.
 */
async function sendInvoiceTestEmail(invoice, owner) {
  if (!invoice) return { ok: false, reason: 'invalid_args' };
  if (!owner || !owner.email) return { ok: false, reason: 'no_owner_email' };
  return sendEmail({
    to: owner.email,
    subject: `[Test] ${buildInvoiceSubject(invoice, owner)}`,
    html: buildInvoiceHtml(invoice, owner),
    text: buildInvoiceText(invoice, owner),
    replyTo: resolveReplyTo(owner)
  });
...
/*
 * Server-sent payment reminder to the CLIENT (Milestone 4 — sent → paid).
 * Fired by POST /invoices/:id/send-reminder when the freelancer taps
 * "Send reminder via DecentInvoice" on the dashboard. Distinct from the
 * dashboard's mailto:/sms:/whatsapp: deep-link buttons, which hand off to
 * the user's local mail/SMS client and frequently dead-end on mobile (iOS
 * without a configured mail account = dead button). This fires from our
 * Resend infrastructure: works on every device, every time. Reply-to
 * points back at the freelancer so a client "I already paid"/"Wrong
 * amount" reply lands in the freelancer's inbox. Polite, short, mobile-
 * friendly. Includes the public /i/<token> share URL so the client can
 * click straight to the invoice page (where the Stripe Payment Link, if
 * any, lives).
 */
function buildPaymentReminderSubject(invoice) {
  const number = (invoice && invoice.invoice_number) || 'your invoice';
  return `Reminder: ${number} is still unpaid`;
}

function buildPaymentReminderHtml(invoice, owner) {
  const number = (invoice && invoice.invoice_number) || '';
  const total = formatMoney(invoice && invoice.total, invoice && invoice.currency);
  const clientName = (invoice && invoice.client_name) || 'there';
  const sender = senderName(owner);
  const url = publicInvoiceUrl(invoice);
  const dueDate = invoice && invoice.due_date ? new Date(invoice.due_date) : null;
  const dueMs = dueDate ? dueDate.getTime() : NaN;
  const overdue = Number.isFinite(dueMs) && dueMs < Date.now();
  const dueLine = Number.isFinite(dueMs)
    ? `<p style="color:#555;margin:8px 0;font-size:14px;"><strong>Due:</strong> ${escapeHtml(dueDate.toISOString().slice(0, 10))}${overdue ? ' (past due)' : ''}</p>`
    : '';
  const viewButton = url
    ? `<p style="margin:24px 0;"><a href="${escapeHtml(url)}" style="background:#2563eb;color:#fff;text-decoration:none;font-weight:600;padding:12px 20px;border-radius:8px;display:inline-block;">View &amp; pay invoice</a></p>`
    : '';
  return `<!doctype html>
<html><body style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#222;background:#f7f7f9;margin:0;padding:24px;">
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #eaeaea;">
  <tr><td style="padding:24px 28px;">
    <h2 style="margin:0 0 8px 0;color:#111;">${overdue ? 'Past due' : 'Friendly reminder'}</h2>
    <p style="color:#222;margin:8px 0;font-size:16px;">Hi ${escapeHtml(clientName)},</p>
    <p style="color:#222;margin:8px 0;font-size:16px;">Just a quick reminder that invoice <strong>${escapeHtml(number)}</strong> for <strong>${escapeHtml(total)}</strong> from <strong>${escapeHtml(sender)}</strong> is still showing as unpaid.</p>
    ${dueLine}
    ${viewButton}
    <p style="color:#555;font-size:14px;margin:16px 0 0 0;">If you&rsquo;ve already paid, just reply to this email and let ${escapeHtml(sender)} know &mdash; the link above lets you also confirm payment from your side.</p>
    <p style="color:#999;font-size:12px;margin-top:24px;">Sent by DecentInvoice on behalf of ${escapeHtml(sender)}. Reply to this email to reach them directly.</p>
  </td></tr>
</table>
</body></html>`;
}

function buildPaymentReminderText(invoice, owner) {
  const number = (invoice && invoice.invoice_number) || '';
  const total = formatMoney(invoice && invoice.total, invoice && invoice.currency);
  const clientName = (invoice && invoice.client_name) || 'there';
  const sender = senderName(owner);
  ...
  const lines = [
    `Hi ${clientName},`,
    '',
    `Just a quick reminder that invoice ${number} for ${total} from ${sender} is still showing as unpaid.`
  ];
  if (Number.isFinite(dueMs)) {
    lines.push('', `Due: ${dueDate.toISOString().slice(0, 10)}${overdue ? ' (past due)' : ''}`);
  }
  if (url) lines.push('', `View & pay: ${url}`);
  lines.push('', `If you've already paid, just reply to this email and let ${sender} know.`);
  lines.push('', `Sent by DecentInvoice on behalf of ${sender}. Reply to this email to reach them directly.`);
  return lines.join('\n');
}

async function sendPaymentReminderEmail(invoice, owner) {
  if (!invoice || !owner) return { ok: false, reason: 'invalid_args' };
  const to = invoice.client_email;
  if (!to) return { ok: false, reason: 'no_client_email' };
  return sendEmail({
    to,
    subject: buildPaymentReminderSubject(invoice),
    html: buildPaymentReminderHtml(invoice, owner),
    text: buildPaymentReminderText(invoice, owner),
    replyTo: resolveReplyTo(owner)
  });
}
...
async function sendWelcomeEmail(user, opts) {
  if (!user || !user.email) return { ok: false, reason: 'no_recipient' };
  const unsubscribeUrl = opts && typeof opts.unsubscribeUrl === 'string' ? opts.unsubscribeUrl : '';
  return sendEmail({
    to: user.email,
...
/*
 * Terminal client-viewed-but-unpaid follow-up (Milestone 4 — sent → paid).
 * Recipient is the FREELANCER. Fires 7+ days after the first client-viewed
 * follow-up when the invoice is STILL unpaid. Empathetic, terminal framing
 * (mirrors second-stale-draft-email): "anything we can help unblock?",
 * archive-suggesting fallback. Stops re-nudging the freelancer beyond this
 * point; the overdue-digest takes over once due_date passes.
 *
 * The CTA bakes in a magic-login URL with ?next=/invoices/<id> so a click
 * lands signed-in on the invoice page where the share-intent + mark-as-paid
 * buttons live. Mint failure soft-falls to /invoices/<id>.
 */
function buildSecondClientViewedFollowupSubject(row) {
  const number = (row && row.invoice_number) || 'your invoice';
  return `Still no payment on ${number}? One last nudge.`;
}

function secondClientViewedFollowupCtaUrl(row, opts) {
  if (!row || row.invoice_id == null) return '';
  const magicLoginUrl = opts && typeof opts.magicLoginUrl === 'string'
    ? opts.magicLoginUrl.trim() : '';
  if (magicLoginUrl) {
...
function daysSinceFirstFollowup(firstFollowupSentAt, now = new Date()) {
  if (!firstFollowupSentAt) return 0;
  const stamped = new Date(firstFollowupSentAt).getTime();
  if (!Number.isFinite(stamped)) return 0;
  const diff = now.getTime() - stamped;
  return Math.max(0, Math.floor(diff / 86400000));
}

function buildSecondClientViewedFollowupHtml(row, now = new Date(), opts) {
  const number = (row && row.invoice_number) || '';
  const clientName = (row && row.client_name) || 'your client';
  const total = formatMoney(row && row.invoice_total);
  const greeting = (row && (row.name || row.business_name)) || 'there';
  const daysSinceNudge = Math.max(7, daysSinceFirstFollowup(row && row.first_followup_sent_at, now));
  const url = secondClientViewedFollowupCtaUrl(row, opts);
  const ctaButton = url
    ? `<p style="margin:24px 0;"><a href="${escapeHtml(url)}" style="background:#4f46e5;color:#fff;text-decoration:none;font-weight:600;padding:12px 20px;border-radius:8px;display:inline-block;">Open invoice &amp; chase</a></p>`
    : '';
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #eaeaea;">
  <tr><td style="padding:24px 28px;">
    <h2 style="margin:0 0 8px 0;color:#111;">Still no payment on invoice ${escapeHtml(number)} from ${escapeHtml(clientName)}</h2>
    <p style="color:#222;margin:8px 0;font-size:16px;">Hi ${escapeHtml(greeting)},</p>
    <p style="color:#222;margin:8px 0;font-size:16px;">We nudged you about <strong>${escapeHtml(clientName)}</strong>&rsquo;s unpaid invoice <strong>${escapeHtml(number)}</strong> (<strong>${escapeHtml(total)}</strong>) about <strong>${daysSinceNudge} days</strong> ago. It&rsquo;s still sitting unpaid &mdash; usually that means one of these:</p>
    <ul style="color:#222;margin:8px 0 16px 20px;font-size:15px;line-height:1.6;">
      <li>The client is waiting on something from you (the scope, a deliverable, an approval).</li>
      <li>You already settled it another way (cash, transfer, swap of services) and forgot to mark it paid here.</li>
      <li>You wrote it off and would rather not chase it any more &mdash; in which case mark it paid or archive it and move on.</li>
      <li>The client is genuinely slow-paying and a firmer "hey, just to flag this is now <em>${daysSinceNudge} days</em> overdue" will move it.</li>
    </ul>
    <p style="color:#222;margin:8px 0;font-size:15px;">If any of those ring a bell, hit reply &mdash; we read every message. Otherwise tap below to open the invoice &amp; fire a one-tap WhatsApp / SMS / Email follow-up:</p>
    ${ctaButton}
    <p style="color:#999;font-size:12px;margin-top:24px;">Sent automatically as the terminal reminder &mdash; we won&rsquo;t nudge you again about this invoice. If it&rsquo;s been paid out-of-band, open the invoice and mark it Paid so we stop bugging you.</p>
  </td></tr>
</table>
</body></html>`;
}

function buildSecondClientViewedFollowupText(row, now = new Date(), opts) {
  const number = (row && row.invoice_number) || '';
  const clientName = (row && row.client_name) || 'your client';
  const total = formatMoney(row && row.invoice_total);
  const greeting = (row && (row.name || row.business_name)) || 'there';
  ...
  const lines = [
    `Hi ${greeting},`,
    '',
    `We nudged you about ${clientName}'s unpaid invoice ${number} (${total}) about ${daysSinceNudge} days ago. `
      + "It's still sitting unpaid — usually that means one of these:",
    '',
    '  - The client is waiting on something from you (scope, deliverable, approval).',
    '  - You already settled it another way and forgot to mark it paid.',
    "  - You wrote it off — in which case mark it paid or archive it.",
    `  - The client is genuinely slow-paying and a firmer "this is now ${daysSinceNudge} days overdue" will move it.`,
    '',
    'If any ring a bell, hit reply. Otherwise:'
  ];
  if (url) {
    lines.push('', `Open invoice & chase: ${url}`);
  }
  lines.push('', "Terminal reminder — we won't nudge you again about this invoice.");
  return lines.join('\n');
}

async function sendSecondClientViewedFollowupEmail(row, opts) {
  if (!row || !row.email) return { ok: false, reason: 'no_recipient' };
  const buildOpts = opts && typeof opts.magicLoginUrl === 'string' && opts.magicLoginUrl
    ? { magicLoginUrl: opts.magicLoginUrl } : undefined;
  const now = (opts && opts.now) || new Date();
...
/*
 * Pending-payment-claim follow-up (Milestone 4 — sent → paid). Recipient is
 * the FREELANCER. Fires 48h+ after a client clicked "I've sent payment" on
 * the public /i/<token> share page when the freelancer still hasn't flipped
 * the invoice to paid. Distinct framing from the original
 * sendPaymentClaimedEmail (which celebrates the client's claim the moment it
 * happens): this is a second-pass nudge ("your client said they paid two days
 * ago — please confirm so they don't think you've ghosted them"). The CTA
 * bakes in a magic-login URL with ?next=/invoices/<id> so the freelancer
 * lands signed-in straight on the invoice page where Mark-as-Paid lives.
 * Mint failure soft-falls to the plain /invoices/<id> path.
 */
function buildPaymentClaimFollowupSubject(row) {
  const number = (row && row.invoice_number) || 'your invoice';
  const clientName = (row && row.client_name) || 'Your client';
  return `Reminder: ${clientName} reported paying ${number} — confirm to close the loop`;
}

function paymentClaimFollowupCtaUrl(row, opts) {
  if (!row || row.invoice_id == null) return '';
  const magicLoginUrl = opts && typeof opts.magicLoginUrl === 'string'
    ? opts.magicLoginUrl.trim() : '';
  if (magicLoginUrl) {
...
function daysSinceClaimed(claimedAt, now = new Date()) {
  if (!claimedAt) return 0;
  const claimed = new Date(claimedAt).getTime();
  if (!Number.isFinite(claimed)) return 0;
  const diff = now.getTime() - claimed;
  return Math.max(0, Math.floor(diff / 86400000));
}

function buildPaymentClaimFollowupHtml(row, now = new Date(), opts) {
  const number = (row && row.invoice_number) || '';
  const clientName = (row && row.client_name) || 'Your client';
  const total = formatMoney(row && row.invoice_total);
  const greeting = (row && (row.name || row.business_name)) || 'there';
  const days = daysSinceClaimed(row && row.payment_claimed_at, now);
  const url = paymentClaimFollowupCtaUrl(row, opts);
  // Cohort gate is 48h+ so floor at "2 days" — never "0 days" / "1 day".
  const dayLabel = days <= 1 ? '2 days' : `${days} days`;
  const method = paymentMethodLabel(row && row.payment_claim_method);
  const reference = (row && row.payment_claim_reference)
    ? String(row.payment_claim_reference).slice(0, 200) : '';
  const note = (row && row.payment_claim_note)
    ? String(row.payment_claim_note).slice(0, 1000) : '';
  const refLine = reference
    ? `<p style="color:#222;margin:8px 0;font-size:15px;"><strong>Reference:</strong> ${escapeHtml(reference)}</p>`
    : '';
  const noteLine = note
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #eaeaea;">
  <tr><td style="padding:24px 28px;">
    <h2 style="margin:0 0 8px 0;color:#111;">⏰ Reminder: ${escapeHtml(clientName)} reported paying ${escapeHtml(number)}</h2>
    <p style="color:#222;margin:8px 0;font-size:16px;">Hi ${escapeHtml(greeting)},</p>
    <p style="color:#222;margin:8px 0;font-size:16px;"><strong>${escapeHtml(clientName)}</strong> clicked &ldquo;I&rsquo;ve sent payment&rdquo; on invoice <strong>${escapeHtml(number)}</strong> (<strong>${escapeHtml(total)}</strong>) about <strong>${escapeHtml(dayLabel)} ago</strong>, but it&rsquo;s still showing unpaid in DecentInvoice.</p>
    <p style="color:#222;margin:8px 0;font-size:15px;"><strong>Method:</strong> ${escapeHtml(method)}</p>
    ${refLine}
    ${noteLine}
    <p style="color:#222;margin:12px 0;font-size:16px;">From your client&rsquo;s side, the next move is yours: verify the funds landed in your account, then mark the invoice paid so they see the &ldquo;Paid&rdquo; banner on their copy of the link. Leaving it unconfirmed for too long suggests you didn&rsquo;t get the message &mdash; not great for the relationship.</p>
    ${ctaButton}
    <p style="color:#999;font-size:12px;margin-top:24px;">Sent once per invoice when a client-reported payment isn&rsquo;t confirmed within 48 hours. You&rsquo;ll only get this reminder once. DecentInvoice cannot verify whether the payment actually cleared — always confirm against your bank / payment app before marking paid.</p>
  </td></tr>
</table>
</body></html>`;
}

function buildPaymentClaimFollowupText(row, now = new Date(), opts) {
  const number = (row && row.invoice_number) || '';
  const clientName = (row && row.client_name) || 'Your client';
  const total = formatMoney(row && row.invoice_total);
  const greeting = (row && (row.name || row.business_name)) || 'there';
  ...
  const lines = [
    `Hi ${greeting},`,
    '',
    `${clientName} clicked "I've sent payment" on invoice ${number} (${total}) about `
      + `${dayLabel} ago, but it's still showing unpaid in DecentInvoice.`,
    '',
    `Method: ${method}`
  ];
  if (reference) lines.push(`Reference: ${reference}`);
  ...
  lines.push(
    '',
    'Verify the funds landed in your account, then mark the invoice paid so your client '
      + 'sees the "Paid" banner on their copy of the link.'
  );
  if (url) {
    lines.push('', `Confirm receipt & mark paid: ${url}`);
  }
  lines.push(
    '',
    'Sent once per invoice when a client-reported payment isn\'t confirmed within 48 hours.'
  );
  return lines.join('\n');
}

async function sendPaymentClaimFollowupEmail(row, opts) {
  if (!row || !row.email) return { ok: false, reason: 'no_recipient' };
  const buildOpts = opts && typeof opts.magicLoginUrl === 'string' && opts.magicLoginUrl
    ? { magicLoginUrl: opts.magicLoginUrl } : undefined;
  const now = (opts && opts.now) || new Date();
...
/*
 * Inactive-user re-engagement email (Milestone 1 — signup → re-entry, for
 * the activated-but-silent cohort). The full activation cron stack fires
 * on invoice-state cohorts; a user who already activated once and went
 * silent for 14+ days falls through every existing job. This one-shot
 * pulls them back with a friendly "anything new to bill?" framing and a
 * magic-login CTA straight to /invoices/quick. No PII in the subject
 * (inbox-preview privacy); copy is plan-agnostic.
 */
function daysSinceLastLogin(lastLoginAt, now) {
  if (!lastLoginAt) return 14;
  const ts = lastLoginAt instanceof Date ? lastLoginAt.getTime() : new Date(lastLoginAt).getTime();
  if (!Number.isFinite(ts)) return 14;
  const ref = now instanceof Date ? now.getTime() : Date.now();
  const days = Math.floor((ref - ts) / 86400000);
  return days < 14 ? 14 : days;
}

function buildInactiveReengagementSubject() {
  return 'Still freelancing? Send your next invoice in 60 seconds.';
}

function inactiveReengagementCtaUrl(opts) {
  const magicLoginUrl = opts && typeof opts.magicLoginUrl === 'string'
    ? opts.magicLoginUrl.trim() : '';
  if (magicLoginUrl) return `${magicLoginUrl}?next=/invoices/quick`;
  const base = (process.env.APP_URL || '').replace(/\/+$/, '');
  return base ? `${base}/invoices/quick` : '';
}

function inactiveReengagementDashboardUrl() {
  const base = (process.env.APP_URL || '').replace(/\/+$/, '');
  return base ? `${base}/invoices` : '';
}

function buildInactiveReengagementHtml(row, now, opts) {
  const greeting = (row && (row.name || row.business_name)) || 'there';
  const days = daysSinceLastLogin(row && row.last_login_at, now);
  const ctaUrl = inactiveReengagementCtaUrl(opts);
  const dashUrl = inactiveReengagementDashboardUrl();
  const ctaButton = ctaUrl
    ? `<p style="margin:24px 0;"><a href="${escapeHtml(ctaUrl)}" style="background:#4f46e5;color:#fff;text-decoration:none;font-weight:600;padding:12px 20px;border-radius:8px;display:inline-block;">Create your next invoice →</a></p>`
    : '';
  const dashLine = dashUrl
    ? `<p style="color:#666;margin:8px 0;font-size:14px;">Or open your dashboard to review past invoices: <a href="${escapeHtml(dashUrl)}" style="color:#4f46e5;">${escapeHtml(dashUrl)}</a></p>`
    : '';
  return `<!doctype html>
<html><body style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#222;background:#f7f7f9;margin:0;padding:24px;">
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #eaeaea;">
  <tr><td style="padding:24px 28px;">
    <h2 style="margin:0 0 8px 0;color:#111;">Anything new to bill, ${escapeHtml(greeting)}?</h2>
    <p style="color:#222;margin:8px 0;font-size:16px;">It&rsquo;s been about ${days} days since you last opened DecentInvoice. If you&rsquo;ve done client work in that window, getting it on a clean invoice today means getting paid this week.</p>
    <p style="color:#222;margin:8px 0;font-size:16px;">The 60-second express form is one tap away &mdash; client name, what you did, the amount, share with your client.</p>
    ${ctaButton}
    ${dashLine}
    <p style="color:#999;font-size:12px;margin-top:24px;">Sent once per inactive user. Reply to this email if anything&rsquo;s blocking you &mdash; we read every reply.</p>
  </td></tr>
</table>
</body></html>`;
}

function buildInactiveReengagementText(row, now, opts) {
  const greeting = (row && (row.name || row.business_name)) || 'there';
  const days = daysSinceLastLogin(row && row.last_login_at, now);
  const ctaUrl = inactiveReengagementCtaUrl(opts);
  const dashUrl = inactiveReengagementDashboardUrl();
  const lines = [
    `Hi ${greeting},`,
    '',
    `It's been about ${days} days since you last opened DecentInvoice. If you've done client work in that window, getting it on a clean invoice today means getting paid this week.`,
    '',
    'The 60-second express form is one tap away — client name, what you did, the amount, share with your client.'
  ];
  if (ctaUrl) lines.push('', `Create your next invoice: ${ctaUrl}`);
  if (dashUrl) lines.push('', `Or open your dashboard: ${dashUrl}`);
  lines.push('', 'Sent once per inactive user. Reply if anything\'s blocking you — we read every reply.');
  return lines.join('\n');
}

async function sendInactiveReengagementEmail(row, opts) {
  if (!row || !row.email) return { ok: false, reason: 'no_recipient' };
  const buildOpts = opts && typeof opts.magicLoginUrl === 'string' && opts.magicLoginUrl
    ? { magicLoginUrl: opts.magicLoginUrl } : undefined;
  const now = (opts && opts.now) || new Date();
  ...
  return sendEmail({
    to: row.email,
    subject: buildInactiveReengagementSubject(row),
