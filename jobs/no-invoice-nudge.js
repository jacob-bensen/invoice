...
function buildNoInvoiceNudgeHtml(row) {
  const greeting = greetingName(row);
  const newUrl = newInvoiceUrl();
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;border:1px solid #eaeaea;">
  <tr><td style="padding:24px 28px;">
    ...
    <p style="color:#222;margin:8px 0;font-size:16px;">You signed up for DecentInvoice a couple of days ago but haven't created your first real invoice yet. It takes under 60 seconds:</p>
    <ul style="color:#222;margin:8px 0 16px 20px;font-size:15px;line-height:1.6;">
      <li>Add your client's name + email</li>
      <li>List what you did + the price</li>
      <li>Mark it Sent — your client gets the link, and you're on the get-paid clock</li>
    </ul>
...
function buildNoInvoiceNudgeText(row) {
  const greeting = greetingName(row);
  const newUrl = newInvoiceUrl();
  const dashUrl = dashboardUrl();
  const lines = [
    `Hi ${greeting},`,
    '',
    'You signed up for DecentInvoice a couple of days ago but haven\'t created your first real invoice yet. It takes under 60 seconds:',
    '',
    '  - Add your client\'s name + email',
    '  - List what you did + the price',
    '  - Mark it Sent — your client gets the link, and you\'re on the get-paid clock',
    '',
  ...
  for (const row of rows) {
    ...
    try {
      result = await sendEmail({
        ...
        html: buildNoInvoiceNudgeHtml(row, now),
        text: buildNoInvoiceNudgeText(row, now),
